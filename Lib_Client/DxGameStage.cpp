...
HRESULT DxGameStage::FrameMove ( float m_fTime, float m_fElapsedTime )
{
	...
	if ( DxInputDevice::GetInstance().GetKeyState(DIK_F8)&DXKEY_UP && GLGaeaClient::GetInstance().GetCharacter()->m_dwUserLvl >= USER_GM3 ) 
	{
		(DxViewPort::GetInstance().IsDefault()) ? DxViewPort::GetInstance().SetGameCamera(), DxFogMan::GetInstance().ManualRangeSet( FALSE, 0.0f, 0.0f ) : DxViewPort::GetInstance().SetDefaultCamera(), GLGaeaClient::GetInstance().GetCharacter()->m_vSAVEPOSGM = GLGaeaClient::GetInstance().GetCharacter()->GetPosition(), DxFogMan::GetInstance().ManualRangeSet( TRUE, 0.0f, 50000.0f );
		GLMSG::SNETPC_GM_CAMERA_ONOFF NetMsg;
	...
	if( DxInputDevice::GetInstance().GetKeyState(DIK_F6)&DXKEY_DOWN )
	{
		...
		if( pCharacter )
		{
			pCharacter->ReqToggleRunAutoPots();
		}
	}
	...
	//
	if ( !DxViewPort::GetInstance().IsDefault() )
	{
		D3DXVECTOR3 vCamera = GLGaeaClient::GetInstance().GetCharacter()->GetPosition();
		vCamera.y += 10.0f;
		DxViewPort::GetInstance().CameraJump ( vCamera );
