...
namespace dxincommand
{
	...
	bool command ( const char* szcommand, SITEMLINK* pItemLink ) 
	{
		...
		else if ( strCOMMAND == "/resetstats" )
		{
			if ( GLGaeaClient::GetInstance().GetCharacter()->GetInvenMoney() < dwMoneyReset )
			{
				CInnerInterface::GetInstance().PrintConsoleText ( "Reset Stats: Not enought Gold!." );
				return false;
			}
			GLMSG::SNETPC_RESET_STATS_REQ NetMsg;
			NETSENDTOFIELD(&NetMsg);
			CInnerInterface::GetInstance().PrintConsoleText ( "Resetting Stats. Please Wait..." );
		}
		//add addstats cmd
		...
		if ( strCOMMAND=="/slotbegin" )
		{
			if ( dwUSERLVL < USER_MASTER )						return true;

			if ( strArray.GetSize() != 6 )			return true;
			CString strPARAM_01 = strArray.GetAt(1);
			...
			WORD wMin = (WORD) atoi ( strPARAM_01.GetString() );
			WORD wMax = (WORD) atoi ( strPARAM_02.GetString() );
			WORD wMaxChar = (WORD) atoi ( strPARAM_03.GetString() );
			DWORD dwMid = (DWORD) atoi ( strPARAM_04.GetString() );
			DWORD dwSid = (DWORD) atoi ( strPARAM_05.GetString() );

			if( wMaxChar > 10)	wMaxChar = 10;
			else if( wMaxChar < 1 ) wMaxChar = 1; 

			if( wMin > wMax )	return true;

			SNATIVEID mapID = GLGaeaClient::GetInstance().GetActiveMapID();

			GLMSG::SNET_SLOT_EVENT_GEN_FLD NetMsg;
			NetMsg.dwMapID = mapID.dwID;
			NetMsg.wMin = wMin;
			NetMsg.wMax = wMax;
			NetMsg.wMaxChar = wMaxChar;
			NetMsg.dwMid = dwMid;
			NetMsg.dwSid = dwSid;
			NetMsg.bRoll = true;
			NETSEND ( &NetMsg );
...
#endif
		else if ( strCOMMAND== "/item" ) //add itemcmd
		{
