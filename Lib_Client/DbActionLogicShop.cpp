...
CUpdateMoneyPoints::CUpdateMoneyPoints( 
	DWORD dwPremium,
	CString strUserID )
{
	m_dwPremium = dwPremium;
	m_strUserID = strUserID;
}

int CUpdateMoneyPoints::Execute( CServer* pServer )
{
	return m_pDbManager->UpdateMoneyPoint ( m_dwPremium, m_strUserID );
}

//PANDORA - Added by Arwekaj09 to Deduct UserPoint or E-Points
CDeductMoneyPoints::CDeductMoneyPoints( 
	DWORD dwPremium,
	CString strUserID )
{
	m_dwPremium = dwPremium;
...
int CDeductMoneyPoints::Execute( CServer* pServer )
{
	return m_pDbManager->DeductMoneyPoint ( m_dwPremium, m_strUserID );
}