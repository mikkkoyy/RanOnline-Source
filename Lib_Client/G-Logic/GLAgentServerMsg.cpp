...
BOOL GLAgentServer::MsgReqFieldSvrCharChkFb ( NET_MSG_GENERIC* nmg, DWORD _dwClientID, DWORD _dwGaeaID )
{
	...
	/*dmk14 ingame web*/
	NetJoinField.dwPremiumPoints	= pChar->m_dwPremiumPoints;
	NetJoinField.dwCombatPoints		= pChar->m_dwCombatPoints;

	NetJoinField.emUserType			= pChar->m_emUserType;
	...
	case CHAT_TYPE_NORMAL: // 일반메시지
		{
			...
			/*dmk14 chat limit*/
			if( pChar->IsPapering() )	return FALSE;

			pChar->m_bChatProc = true;
			pChar->m_wChatCount++;
...
BOOL GLAgentServer::MsgPartySearchLure ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	GLMSG::SNET_PARTY_SEARCH_LURE* pNetMsg = (GLMSG::SNET_PARTY_SEARCH_LURE*) nmg;

	PGLCHARAG pCharMaster = GetChar ( pNetMsg->dwGaeaID );
	if ( !pCharMaster )							return FALSE;
	DWORD dwPartyID = pCharMaster->m_dwPartyID;

	PGLCHARAG pCharRequestor = GetChar ( dwGaeaID );
	if ( !pCharRequestor )						return FALSE;

	if ( pCharRequestor->m_dwPartyID!=PARTY_NULL )
		return FALSE;

	if (pCharRequestor)
	{
		GLAGLandMan* pLandMan = GetByMapID ( pCharRequestor->m_sCurMapID );
		if( pLandMan && pLandMan->IsInstantMap() ) 
		{
			GLMSG::SNET_PARTY_SEARCH_LURE_REQUESTOR_FB NetMsgFB;
			NetMsgFB.dwMasterGaeaID = pNetMsg->dwGaeaID;
			StringCchCopy ( NetMsgFB.m_szName, CHAR_SZNAME, pCharMaster->m_szName );
			NetMsgFB.emAnswer = EMPARTY_LURE_REFUSE;

			SENDTOCLIENT ( pCharRequestor->m_dwClientID, &NetMsgFB );
			return FALSE;
		}
	}
	...
	if ( bCaptureTheFlag && pCharMaster && pCharRequestor )
	{
		if ( pCharMaster->m_wCaptureTheFlagTeam != pCharRequestor->m_wCaptureTheFlagTeam )
		{
			//Master's Info
			GLMSG::SNET_PARTY_SEARCH_LURE_REQUESTOR_FB NetMsgFB;
			NetMsgFB.dwMasterGaeaID = pNetMsg->dwGaeaID;
	...
	{
		if ( pCharRequestor->m_wSchool!=pCharMaster->m_wSchool )
		{
			//Master's Info
			GLMSG::SNET_PARTY_SEARCH_LURE_REQUESTOR_FB NetMsgFB;
			NetMsgFB.dwMasterGaeaID = pNetMsg->dwGaeaID;
	...
	{
		//ptcheatfix
		pCharMaster->m_mapPartyReq[pCharRequestor->m_dwCharID] = pCharRequestor->m_dwCharID;
		return FALSE;;
	}

	GLPARTY* pParty = m_cPartyMan.GetParty ( dwPartyID );
	if ( !pParty )								return FALSE;
	if ( !pParty->ISMASTER(pNetMsg->dwGaeaID) )			return FALSE;
	...
	pParty->PREADDMEMBER ( pCharRequestor->m_dwGaeaID );

	GLMSG::SNET_PARTY_SEARCH_LURE_FB NetMsgLureFB;
	StringCchCopy ( NetMsgLureFB.m_szName, CHAR_SZNAME, pCharRequestor->m_szName ); 
	NetMsgLureFB.dwGaeaID = pCharRequestor->m_dwGaeaID;
	NetMsgLureFB.m_emClass = pCharRequestor->m_emClass;
	NetMsgLureFB.m_wLevel = pCharRequestor->m_wLevel;
	NetMsgLureFB.m_wSchool = pCharRequestor->m_wSchool;
	SENDTOCLIENT ( pCharMaster->m_dwClientID, &NetMsgLureFB );

	return TRUE;
}
BOOL GLAgentServer::MsgPartyLureSearchTarAns ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	GLMSG::SNET_PARTY_SEARCH_LURE_TAR_ANS* pNetMsg = (GLMSG::SNET_PARTY_SEARCH_LURE_TAR_ANS*) nmg;

	PGLCHARAG pCharMaster = GetChar ( dwGaeaID );
	if ( !pCharMaster )	return FALSE;
	
	PGLCHARAG pCharTar = GetChar ( pNetMsg->dwRequestorID );
	if ( !pCharTar ) return FALSE;

	if ( !GLCONST_CHAR::bPARTY_2OTHERSCHOOL && pCharTar->m_wSchool != pCharMaster->m_wSchool )		return FALSE;
	if ( pCharTar->GetPartyID() != PARTY_NULL)  
	{
		GLMSG::SNET_PARTY_SEARCH_LURE_REQUESTOR_FB NetMsgFB;
		NetMsgFB.dwMyGaeaID = pCharTar->m_dwGaeaID;
		StringCchCopy ( NetMsgFB.m_szName, CHAR_SZNAME, pCharTar->m_szName );
	...
	if ( pCharMaster->GetPartyID() == PARTY_NULL )
	{
		GLMSG::SNET_PARTY_SEARCH_LURE_REQUESTOR_FB NetMsgFB;
		NetMsgFB.emAnswer = EMPARTY_LURE_NOT_AVAILABLE;
		SENDTOCLIENT ( pCharMaster->m_dwClientID, &NetMsgFB );

		return FALSE;
	}

	if ( pNetMsg->dwRequestorID == pCharTar->m_dwGaeaID )
	{
		GLPARTY* pParty = m_cPartyMan.GetParty ( pCharMaster->GetPartyID() );
		if ( !pParty )									return FALSE;
		if ( pParty->ISFULL() )							return FALSE;

		pParty->PREDELMEMBER ( pCharTar->m_dwGaeaID );
		...
		/*pvp capture the flag, Juver, 2018/02/03 */
		BOOL bCaptureTheFlag = FALSE;
		GLAGLandMan* pLandMan = GetByMapID ( pCharTar->m_sCurMapID );

		if( pLandMan && pLandMan->IsInstantMap() ) return FALSE;
		...
		if ( pNetMsg->emAnswer == EMPARTY_LURE_OK )
		{
			...
			if ( pParty )
			{
				...
				pParty->ADDMEMBER ( pCharTar->m_dwGaeaID );

				//	Note : 필드 서버에 파티원이 추가됨을 알림.
				//
				GLMSG::SNET_PARTY_ADD NetPartyAdd;
				NetPartyAdd.dwPartyID = pCharMaster->GetPartyID();
				NetPartyAdd.sPartyInfo = GLPARTY_FNET(pCharTar->m_dwGaeaID,pCharTar->m_szName,pCharTar->m_emClass,pCharTar->m_sCurMapID,pCharTar->m_wSchool,pCharTar->m_wLevel);
		...
		GLMSG::SNET_PARTY_SEARCH_LURE_REQUESTOR_FB NetMsgFB;
		StringCchCopy ( NetMsgFB.m_szName, CHAR_SZNAME, pCharMaster->m_szName );
		NetMsgFB.dwMasterGaeaID = pCharMaster->m_dwGaeaID;
		NetMsgFB.emAnswer = pNetMsg->emAnswer;
		SENDTOCLIENT ( pCharTar->m_dwClientID, &NetMsgFB );
	}
...
BOOL GLAgentServer::MsgPartyLureTarAns ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	if ( pNetMsg->dwPartyID == PARTY_NULL && pCharMaster->GetPartyID() == PARTY_NULL )
	{
		...
		for ( ; iter!=iter_end; ++iter )
		{
			...
			NetPartyInfo.ADDMEMBER ( GLPARTY_FNET(pCharMem->m_dwGaeaID,pCharMem->m_szName,pCharMem->m_emClass,pCharMem->m_sCurMapID,pCharMem->m_wSchool,pCharMem->m_wLevel) );
		}
...
BOOL GLAgentServer::MsgGmGenChar ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	/*dmk14 summon whole party or single player*/
	if( pNetMsg->wType == 1 )
	{
		GLPARTY *pParty = GetParty ( pGenCHAR->GetPartyID() );
		if ( pParty )		
		{
			GLPARTY::MEMBER_ITER iter = pParty->m_cMEMBER.begin();
			GLPARTY::MEMBER_ITER iter_end = pParty->m_cMEMBER.end();
			for ( ; iter!=iter_end; ++iter )
			{
				...
				if ( pCharMem->m_dwCurFieldSvr==FIELDSERVER_MAX )
				{
					NetMsgFB.emFB = EMGM_MOVE2CHAR_FB_TO_CONDITION;
					SENDTOCLIENT ( dwClientID, &NetMsgFB );
					continue;
				...
				if ( pCharMem->m_nChannel!=pMyChar->m_nChannel )
				{
					NetMsgFB.emFB = EMGM_MOVE2CHAR_FB_CHANNEL;
					NetMsgFB.nChannel = pCharMem->m_nChannel;
					SENDTOCLIENT ( dwClientID, &NetMsgFB );
					continue;
				}

				GLMSG::SNETPC_GM_MOVE2CHAR_POS NetMsgCK;
				NetMsgCK.dwCOMMAND_CHARID = pCharMem->m_dwCharID;
	...
	else	/*dmk14 default gm command genchar*/
	{
		GLMSG::SNETPC_GM_MOVE2CHAR_POS NetMsgCK;
		NetMsgCK.dwCOMMAND_CHARID = pGenCHAR->m_dwCharID;
		NetMsgCK.dwTO_CHARID = pMyChar->m_dwCharID;
...
HRESULT GLAgentServer::MsgProcess ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	/*PartySearch - Added By Bubu*/
	case NET_MSG_GCTRL_GET_PARTYLIST:					ReqSearchPartyList(nmg, dwClientID, dwGaeaID);	break;
	case NET_MSG_GCTRL_REQ_PARTY_SEARCH_UPDATE:			MsgReqPartySearchUpdate(nmg);					break;
	...
	/*PartySearch - Added By Bubu*/
	case NET_MSG_GCTRL_PARTY_SEARCH_LURE:		MsgPartySearchLure ( nmg, dwClientID, dwGaeaID );			break;
	case NET_MSG_GCTRL_PARTY_SEARCH_LURE_TAR_ANS:		MsgPartyLureSearchTarAns ( nmg, dwClientID, dwGaeaID );	break;
...
/*PartySearch - Added By Bubu*/
BOOL GLAgentServer::ReqSearchPartyList(NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID)
{
	PGLCHARAG pCharAG = GetChar(dwGaeaID);
	if (!pCharAG) return FALSE;

	//Enable if u have lg-7 ranking view
	//if (!pCharAG->m_bCharView)
	//{
	//	GLMSG::SNET_GET_PARTYLIST_FB NetMsgFB;
	//	NetMsgFB.emFB = EMREQ_PARTYLIST_FB_PARTY_DELAY;
	//	SENDTOCLIENT( dwClientID, &NetMsgFB );
	...
	MAPPARTYSEARCH_ITER pos = m_mapPartySearch.begin();
	MAPPARTYSEARCH_ITER end = m_mapPartySearch.end();

	GLMSG::SNET_GET_PARTYLIST_FB NetMsgFB;
	NetMsgFB.emFB = EMREQ_PARTYLIST_FB_CLEAR;
	SENDTOCLIENT( dwClientID, &NetMsgFB );

	NetMsgFB.emFB = EMREQ_PARTYLIST_FB_ADD;

	for ( ; pos != end; ++pos )
	{
		PARTYLINKDATA& sParty = pos->second;

		/*Restrictions before sending to client*/
		if ( sParty.m_wMembers >= MAXPARTY ) continue;
		if( sParty.dwIndex >= MAX_AUCTION_ITEMS )	break;
		GLAGLandMan* pLAND = GetByMapID( sParty.m_sMapID );
		if( pLAND && pLAND->IsInstantMap() )	continue;


		NetMsgFB.sParty = sParty;
		SENDTOCLIENT( dwClientID, &NetMsgFB );
	}

	NetMsgFB.emFB = EMREQ_PARTYLIST_FB_END;
	SENDTOCLIENT( dwClientID, &NetMsgFB );

	return TRUE;
}
BOOL GLAgentServer::MsgReqPartySearchUpdate(NET_MSG_GENERIC* nmg)
{
	GLMSG::SNETPC_REQ_PARTY_SEARCH_UPDATE* pNetMsg = (GLMSG::SNETPC_REQ_PARTY_SEARCH_UPDATE*)nmg;

	VECPARTYSEARCH_ITER iter		=	m_vecPartySearch.begin();
	VECPARTYSEARCH_ITER iter_end	=	m_vecPartySearch.end();

	if (pNetMsg->bDelete)
	{
		for (; iter != iter_end; iter++)
		{
			if ((*iter).dwPartyID == pNetMsg->sParty.dwPartyID)
			{
				m_vecPartySearch.erase ( std::remove(m_vecPartySearch.begin(),m_vecPartySearch.end(),pNetMsg->sParty), m_vecPartySearch.end() );
			}
		}
	}
	else
	{
		...
		for (; iter != iter_end; iter++)
		{
			if ((*iter).dwPartyID == pNetMsg->sParty.dwPartyID)
			{
				...
				if ((*iter).m_dwGaeaID != pNetMsg->sParty.m_dwGaeaID)
					(*iter).m_dwGaeaID = pNetMsg->sParty.m_dwGaeaID;

				if ((*iter).m_wSchool != pNetMsg->sParty.m_wSchool)
					(*iter).m_wSchool = pNetMsg->sParty.m_wSchool;

				if ((*iter).m_wLevel != pNetMsg->sParty.m_wLevel)
					(*iter).m_wLevel = pNetMsg->sParty.m_wLevel;

				if ((*iter).m_emClass != pNetMsg->sParty.m_emClass)
					(*iter).m_emClass = pNetMsg->sParty.m_emClass;

				if ((*iter).m_wMembers != pNetMsg->sParty.m_wMembers)
					(*iter).m_wMembers = pNetMsg->sParty.m_wMembers;

				if ((*iter).m_sMapID != pNetMsg->sParty.m_sMapID)
					(*iter).m_sMapID = pNetMsg->sParty.m_sMapID;

				if ((*iter).sOption != pNetMsg->sParty.sOption)
					(*iter).sOption = pNetMsg->sParty.sOption;

				if (strcmp((*iter).m_szName, pNetMsg->sParty.m_szName) != 0)
					StringCchCopy((*iter).m_szName, CHAR_SZNAME, pNetMsg->sParty.m_szName);
		...
		{
			PARTYLINKDATA sTEMP;
			sTEMP.m_emClass = pNetMsg->sParty.m_emClass;
			sTEMP.m_sMapID = pNetMsg->sParty.m_sMapID;
			sTEMP.m_wSchool = pNetMsg->sParty.m_wSchool;
			sTEMP.sOption = pNetMsg->sParty.sOption;
			sTEMP.dwPartyID = pNetMsg->sParty.dwPartyID;
			sTEMP.m_dwGaeaID = pNetMsg->sParty.m_dwGaeaID;
			sTEMP.m_wMembers = pNetMsg->sParty.m_wMembers;
			StringCchCopy ( sTEMP.m_szName, CHAR_SZNAME, pNetMsg->sParty.m_szName );
			m_vecPartySearch.push_back(sTEMP);
		}
	}
...
BOOL GLAgentServer::MsgReqGlobalRanking(NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID)
{
	...
	for (int i = 0; i < 10; i++)
	{
		VEC_TOP_KILL vecTopKill = m_vecTopKill[i];
		...
		if ( !vecTopKill.empty() )
		{
			...
			{
				for (size_t j = 0; j < vecTopKill.size() ; j++)
				{
					if( j >= MAX_TOP_RANK )	break;
					GLMSG::SNETPC_REQ_GLOBAL_RANKING_KILL_AGT NetMsgFB;
					NetMsgFB.emFB = EM_GLOBAL_RANKING_FB_TOP_KILL + i;
					NetMsgFB.wPart = i;
					NetMsgFB.sTopKill = vecTopKill[j];
					NetMsgFB.nIndex = j;

					PGLCHARAG pCharAG2 = GetCharID(vecTopKill[j].dwChaNum);
					if (!pCharAG2)	NetMsgFB.sTopKill.bChaOnline = FALSE;
					else			NetMsgFB.sTopKill.bChaOnline = TRUE;
					...
					if (bGuild && vecTopKill[j].wGuNum > 0)
					{
						...
						if (pCLUB)
						{
							NetMsgFB.sTopKill.wGuMarkVer = (WORD)pCLUB->m_dwMarkVER;
							StringCchCopy(NetMsgFB.sTopKill.szGuName, CHAR_SZNAME, pCLUB->m_szName);
						}
					}
...
BOOL GLAgentServer::MsgReqGlobalRankingKillUpdate(NET_MSG_GENERIC* nmg)
{
	...
	switch (pNetMsg->sTopKill.nChaClass)
	{
	case 1:
	case 64:
		nNum = 1;
...

	{
		...
		{
			BOOL bFound					= FALSE;
			VEC_TOP_KILL_ITER iter		= m_vecTopKill[0].begin();
			VEC_TOP_KILL_ITER iter_end	= m_vecTopKill[0].end();
			...
			if (!bFound)
			{
				...
				m_vecTopKill[0].push_back( sTEMP );
			}
		}
		...
		{
			BOOL bFound					= FALSE;
			VEC_TOP_KILL_ITER iter		= m_vecTopKill[nNum].begin();
			VEC_TOP_KILL_ITER iter_end	= m_vecTopKill[nNum].end();
			...
			if (!bFound)
			{
				...
				m_vecTopKill[nNum].push_back(sTEMP);
			}
		}
	}
