...
namespace GLMSG
{
	...
	/////////////////////////////////////////////////////////////////////////////
	// LG-7 CWRanking
	struct SNET_REQ_CW_RANKING
	{
		NET_MSG_GENERIC	nmg;

		SNET_REQ_CW_RANKING()
		{
			nmg.dwSize	= sizeof(SNET_REQ_CW_RANKING);
			nmg.nType	= NET_MSG_GCTRL_REQ_CW_RANKING;
			GASSERT(nmg.dwSize <= NET_DATA_BUFSIZE);
		}
	};
	struct SNET_REQ_CW_RANKING_FB
	{
		NET_MSG_GENERIC	nmg;

		DWORD			dwID;
		SCW_RANKING		sCWRanking_[10];

		SNET_REQ_CW_RANKING_FB()
			: dwID(0)
		{
			nmg.dwSize	= sizeof(SNET_REQ_CW_RANKING_FB);
			nmg.nType	= NET_MSG_GCTRL_REQ_CW_RANKING_FB;
			GASSERT(nmg.dwSize <= NET_DATA_BUFSIZE);
		}
	};
	struct SNET_REQ_MY_CW_RANKING
	{
		NET_MSG_GENERIC	nmg;

		SNET_REQ_MY_CW_RANKING()
		{
			nmg.dwSize	= sizeof(SNET_REQ_MY_CW_RANKING);
			nmg.nType	= NET_MSG_GCTRL_REQ_MY_CW_RANKING;
			GASSERT(nmg.dwSize <= NET_DATA_BUFSIZE);
		}
	};
	struct SNET_REQ_MY_CW_RANKING_FB
	{
		NET_MSG_GENERIC	nmg;

		SCW_RANKING		sCWRanking;

		SNET_REQ_MY_CW_RANKING_FB()
		{
			nmg.dwSize	= sizeof(SNET_REQ_MY_CW_RANKING_FB);
			nmg.nType	= NET_MSG_GCTRL_REQ_MY_CW_RANKING_FB;
			GASSERT(nmg.dwSize <= NET_DATA_BUFSIZE);
		}
	};
	/////////////////////////////////////////////////////////////////////////////
