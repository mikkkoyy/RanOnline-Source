...
#include "../Lib_ClientUI/Interface/LargeMapWindowImage.h"
#include "../Lib_ClientUI/Interface/InfoBoxContents.h"
...
void GLCharacter::ReqGateOut ()
{
	...
	if ( emReqFail != EMREQUIRE_COMPLETE )
	{
		...
		switch ( emReqFail )
		{
			/*map requirement school, CNDev 2021/11/02*/
		case EMREQUIRE_SCHOOL:
			{
				cINTERFACE.PrintMsgTextDlg( NS_UITEXTCOLOR::DISABLE, 
					ID2GAMEINTEXT("EMREQUIRE_SCHOOL"),	GLCONST_CHAR::strSCHOOLNAME[pRequire->m_wLevel] );
			}
			break;
...
HRESULT GLCharacter::ReqStorageOpenCardInvenRequest ( WORD wPosX, WORD wPosY )
{
	...
	//ask question
	DoModal( ID2GAMEINTEXT("INVEN_LOCKER_OPEN_ITEM_REQ"), MODAL_QUESTION, YESNO, MODAL_LOCKER_CONNECT_REQ );
...
HRESULT GLCharacter::ReqStorageOpenCardInvenConfirmed ()
{
	if ( CInnerInterface::GetInstance().IsStorageWindowOpen () ) return E_FAIL;

	SINVENITEM* pInvenItem = m_cInventory.FindPosItem ( m_wInvenPosX4, m_wInvenPosY4 );
	if ( !pInvenItem )	return E_FAIL;
...
HRESULT GLCharacter::ReqInvenDrug ( WORD wPosX, WORD wPosY )
{
	...
	if ( pItem->sBasicOp.emItemType == ITEM_CURE )
	{
		...
		if ( pItem->sDrugOp.emDrug!=ITEM_DRUG_NONE )
		{
			...
			case ITEM_DRUG_CP:
				if ( m_sCombatPoint.dwMax == m_sCombatPoint.dwNow )		return S_FALSE;
				break;
			};
	...
	if ( !pHold )
	{
		...
		{
			//Select Box
			if( pItem->sBox.m_bSelect )	CInnerInterface::GetInstance().ShowBoxContents( pInvenItem->sItemCustom.sNativeID, pItem->sBox.m_bSelect, wPosX, wPosY );
			else ReqBoxOpen ( wPosX, wPosY );
		}
		...
		else if ( pItem->sBasicOp.emItemType == ITEM_STORAGE_LOCK_ENABLE )
		{
			ReqEnableStorageLock ( wPosX, wPosY );
		}
		/*storage lock, Juver, 2018/01/13 */
		else if ( pItem->sBasicOp.emItemType == ITEM_STORAGE_LOCK_RECOVER )
		{
			ReqRecoverStorageLockPassword ( wPosX, wPosY );
		}
		/*inventory lock, Juver, 2018/01/13 */
		else if ( pItem->sBasicOp.emItemType == ITEM_INVENTORY_LOCK_ENABLE )
		{
			ReqEnableInventoryLock ( wPosX, wPosY );
		}
		/*inventory lock, Juver, 2018/01/13 */
		else if ( pItem->sBasicOp.emItemType == ITEM_INVENTORY_LOCK_RECOVER )
		{
			ReqRecoverInventoryLockPassword ( wPosX, wPosY );
		}
		else if ( pItem->sBasicOp.emItemType==ITEM_RESET_BRIGHT_CARD )
		{
			ResetBrightPoint ( wPosX, wPosY );
		}
		else if ( pItem->sBasicOp.emItemType==ITEM_EPOINT_CARD )
		{
			AddEPoint ( wPosX, wPosY );
		}
	...
	else	
	{
		...
		else if ( pHold->sBasicOp.emItemType == ITEM_MYSTERY_KEY ) //Mystery Box
		{
			ReqMysteryBox ( wPosX, wPosY );
		}
		else if ( pHold->sBasicOp.emItemType == ITEM_SKILL_REFORMCARD ) //Weapon Skill Reform Card - SqueeeAK
		{
			ReqWeaponReform( wPosX, wPosY );
		}
	}
...
HRESULT GLCharacter::ReqToggleRunAutoPots()
{
	HRESULT hr = S_OK;
	...
	{
		m_bRunAutoPots = true;
		CInnerInterface::GetInstance().PrintMsgText (NS_UITEXTCOLOR::GREEN_WOW, "Auto-pots start" );
	}
	else				
	{
		m_bRunAutoPots = false;
		CInnerInterface::GetInstance().PrintMsgText (NS_UITEXTCOLOR::DISABLE, "Auto-pots stop" );
	}
...
HRESULT GLCharacter::ReqBoxOpen ( WORD wPosX, WORD wPosY )
{
	...
	//Select Box
	if( CInnerInterface::GetInstance().IsVisibleGroup ( INFO_BOX_CONTENTS ) )
		CInnerInterface::GetInstance().HideGroup( INFO_BOX_CONTENTS );
	...
	{
		for ( int i=0; i<ITEM::SBOX::ITEM_SIZE; ++i )
			NetMsg.bSelect[i] = CInnerInterface::GetInstance().GetInfoBoxContents()->GetSelectedIndex(i);
	}
...
HRESULT GLCharacter::ReqRandumBoxOpen ( WORD wPosX, WORD wPosY )
{
	...
	//Mystery Box
	bool bS = pItem->sBasicOp.emItemType == ITEM_MYSTERY_BOX;
	if (bS)
	{
		if ( pInvenItem->sItemCustom.isBoxLock )
		{
			CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_RANDOMBOXOPEN_FB_LOCK") );
			return E_FAIL;
		}
	}
...
void GLCharacter::ReqStatsUpCmd( EMSTATS emStats, DWORD value )
{
	...
	switch( emStats )
	{
		case EMPOW: CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, "Adding %d Pow.", value );	break;
		case EMSTR: CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, "Adding %d Vit.", value );	break;
		case EMSPI: CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, "Adding %d Int.", value );	break;
		case EMDEX: CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, "Adding %d Dex.", value );	break;
		case EMSTA: CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, "Adding %d Agi.", value );	break;
	}
...
/*dmk14 reform*/
HRESULT GLCharacter::ReqRebuildMoveItemSealed()
{
	if ( !IsValidBody() )	return S_FALSE;
	...
	if( m_sPreInventoryItem.VALID() )
	{
		...
		{
			NetMsg.sItemSeal = sPreItem.sNativeID;
		}
		m_sPreInventoryItem.RESET();
	}
...
HRESULT GLCharacter::ReqRebuildResult()
{
	...
	if( cItemRebuild )
	{
		for( int i=0;i<5;i++) NetMsg.emLockOpt[i] = cItemRebuild->GetLockOpt( i );
	}
...
HRESULT GLCharacter::ReqRebuildPREV()
{
	//return S_FALSE;
	...
	GLMSG::SNET_REBUILD_RESULT NetMsg;
	NetMsg.dwNPCID = m_dwNPCID;
	NetMsg.emResult = EMREBUILD_RESULT_PREVIOUS;	
	NETSENDTOFIELD( &NetMsg );
...
HRESULT GLCharacter::ReqPartyFinderInfo ()
{
	GLMSG::SNET_GET_PARTYLIST NetMsg;
	NETSEND ( &NetMsg );
...
HRESULT GLCharacter::ResetBrightPoint( WORD wPosX, WORD wPosY )
{
	SINVENITEM* pInvenItem = m_cInventory.FindPosItem ( wPosX, wPosY );
	if ( !pInvenItem )	
	{
		CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_CARD_BRIGHT_FB_NOITEMINVEN") );	
		return E_FAIL;
	}
	...
	if ( !pItem )	
	{
		CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_CARD_BRIGHT_FB_NOITEMDATA") );	
		return E_FAIL;
	}

	if ( pItem->sBasicOp.emItemType != ITEM_RESET_BRIGHT_CARD )		
	{
		CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_CARD_BRIGHT_FB_NOITEMTYPE") );	
		return E_FAIL;
	}

	DoModal( ID2GAMEINTEXT("EMINVEN_CARD_BRIGHT_FB_MODAL"),  MODAL_INFOMATION, YESNO, MODAL_RESET_BRIGHT );
...
HRESULT GLCharacter::ReqResetBrightPoint ()
{
	GLMSG::SNETPC_CARD_BRIGHT NetMsg;
	NetMsg.wPosX = m_wInvenPosX1;
	NetMsg.wPosY = m_wInvenPosY1;
	NETSENDTOFIELD ( &NetMsg );
...
HRESULT GLCharacter::AddEPoint( WORD wPosX, WORD wPosY )
{
	SINVENITEM* pInvenItem = m_cInventory.FindPosItem ( wPosX, wPosY );
	if ( !pInvenItem )	
	{
		CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_CARD_EPOINT_FB_NOITEMINVEN") );	
		return E_FAIL;
	}
	...
	if ( !pItem )	
	{
		CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_CARD_EPOINT_FB_NOITEMDATA") );	
		return E_FAIL;
	}

	if ( pItem->sBasicOp.emItemType != ITEM_EPOINT_CARD )		
	{
		CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_CARD_EPOINT_FB_NOITEMTYPE") );	
		return E_FAIL;
	}

	DoModal( ID2GAMEINTEXT("EMINVEN_CARD_EPOINT_FB_MODAL"),  MODAL_INFOMATION, YESNO, MODAL_ADD_EPOINT );
...
HRESULT GLCharacter::ReqAddEPoint ()
{
	GLMSG::SNETPC_CARD_EPOINT NetMsg;
	NetMsg.wPosX = m_wInvenPosX1;
	NetMsg.wPosY = m_wInvenPosY1;
	NETSENDTOFIELD ( &NetMsg );
...
//Mystery Box
HRESULT GLCharacter::ReqMysteryBox(WORD wPosX, WORD wPosY)
{
	if ( !IsValidBody() )						return E_FAIL;
	if ( GLTradeClient::GetInstance().Valid() )	return E_FAIL;
...
HRESULT GLCharacter::ReqWeaponReform ( WORD wPosX, WORD wPosY )
{
	if ( !IsValidBody() )						return E_FAIL;
	if ( GLTradeClient::GetInstance().Valid() )	return E_FAIL;
	...
	GLMSG::SNET_INVEN_REFORM_WEAPON NetMsg;
	NetMsg.wPosX = wPosX;
	NetMsg.wPosY = wPosY;
...
/////////////////////////////////////////////////////////////////////////////
// LG-7 CWRanking
HRESULT GLCharacter::ReqCWRanking()
{
	GLMSG::SNET_REQ_CW_RANKING NetMsg;
	NETSENDTOFIELD(&NetMsg);

	return S_OK;
}
HRESULT GLCharacter::ReqMyCWRanking()
{
	GLMSG::SNET_REQ_MY_CW_RANKING NetMsg;
	NETSENDTOFIELD(&NetMsg);
