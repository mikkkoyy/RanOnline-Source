...
float GLCrow::CalculateItemGenRate( float fGenRate, PGLCHAR pChar, bool bParty )
{
	...
	if( pChar )
	{
		...
		{
			SSYSTEM_BUFF &ssystem_buff = pChar->m_sSYSTEM_BUFF[i];
			if ( ssystem_buff.sNATIVEID==NATIVEID_NULL() )	continue;

			PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( ssystem_buff.sNATIVEID );
			if ( !pSkill )													continue;
			if ( !pSkill->m_sAPPLY.IsSpec( EMSPECA_ITEMDROP_RATE ) && !pSkill->m_sAPPLY.IsSpec( EMSPECA_SPECIAL_ITEM_DROP ) )			continue;

			for( int nSpec=0; nSpec<SKILL::MAX_SPEC; ++nSpec )
			{
				if( pSkill->m_sAPPLY.sSpecs[i].emSPEC != EMSPECA_ITEMDROP_RATE &&
					pSkill->m_sAPPLY.sSpecs[i].emSPEC != EMSPECA_SPECIAL_ITEM_DROP )	continue;
