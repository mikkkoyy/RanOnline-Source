...
HRESULT GLLandManClient::Render_MobItem ( CLIPVOLUME &CV )
{
	...
	for ( ; pCharCur; pCharCur = pCharCur->pNext )
	{
		...
		if( !GLCONST_CHAR::bBATTLEROYAL )
		{
			if ( pChar->IsVisibleDetect() && COLLISION::IsCollisionVolume(CV,pChar->m_vMax,pChar->m_vMin) )
			{
				...
				if ( !bCOLL )
				{
					...
					if ( RANPARAM::bHideDeadName )
					{
						if ( pChar->IsDie() )
						{
							if ( m_bPVPTyrannyMap )
							{
								if ( cMYCHAR.m_wSchool == pChar->GetCharData().wSchool )
								{
									if ( pDISP_NAME_MAN )	pDISP_NAME_MAN->ADD_DISP_NAME ( sDISP_NAME );
								}
							}
							else if ( m_bClubBattle || m_bClubBattleHall )
							{
								if ( cMYCHAR.m_dwGuild == pChar->GetCharData().dwGuild ||
									cMYCHAR.m_sCLUB.IsAllianceGuild(pChar->GetCharData().dwGuild) )
								{
									if ( pDISP_NAME_MAN )	pDISP_NAME_MAN->ADD_DISP_NAME ( sDISP_NAME );
								}

							}
							else if ( m_bClubDeathMatch || m_bClubDeathMatchHall )
							{
								if ( cMYCHAR.m_dwGuild == pChar->GetCharData().dwGuild  )
								{
									if ( pDISP_NAME_MAN )	pDISP_NAME_MAN->ADD_DISP_NAME ( sDISP_NAME );
		...
		//cndev hide dead body 12-10-2022
		if ( pChar->IsDie() )
		{
			if ( RANPARAM::bHideDeadBody )
			{
				if ( m_bPVPTyrannyMap )
				{
					if ( cMYCHAR.m_wSchool == pChar->GetCharData().wSchool )
