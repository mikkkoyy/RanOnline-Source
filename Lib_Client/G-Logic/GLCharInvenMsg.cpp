		...
		for ( ; iter!=ItemMap.end(); ++iter )
		{
			...
			else
			{
				...
				if ( pParty )
				{
					...
					NetMsg.sITEMCUSTOM = pItemDrop->sItemCustom;
			...
			else
			{
...

				{
					...
					if ( pItem )
					{
						...
						CString strName;
						strName.Format ( "%d.csv",m_dwUserID );
...
void GLChar::InsertMoney ( LONGLONG lnAmount )
{
...
#endif
	{
		...
		/*dmk14 ingame web*/
		NetMsg.dwPremium = m_dwPremiumPoints;
		NetMsg.dwCombat = m_dwCombatPoints;
		GLGaeaServer::GetInstance().SENDTOCLIENT ( m_dwClientID, &NetMsg );
	}
...
LONGLONG GLChar::CalculateInsertmoney( LONGLONG lnAmount )
{
	...
	for ( int i=0; i<SYSTEM_BUFF_SIZE; ++i )
	{
		...
		if ( !pSkill )													continue;
		if ( !pSkill->m_sAPPLY.IsSpec( EMSPECA_MONEYDROP_RATE ) )		continue;

		for( int nSpec=0; nSpec<SKILL::MAX_SPEC; ++nSpec )
		{
			if( pSkill->m_sAPPLY.sSpecs[i].emSPEC != EMSPECA_MONEYDROP_RATE )	continue;
...
HRESULT GLChar::MsgReqInvenToHold ( NET_MSG_GENERIC* nmg )
{
...

	{
		...
		if ( pItem )
		{
			char szTempChar[256] = {0,};
			string str;
			sprintf( szTempChar, ",[%s][%s], Hold Item from inventory %02d_%02d, %s x(%d/%d), GenNum(%d)", m_szUID, m_szName, 
...
HRESULT GLChar::MsgReqHoldArray ( NET_MSG_GENERIC* nmg )
{
	...
	if ( m_fInvenSortTimer < INVEN_SORT_TIMER )	return E_FAIL;
...
HRESULT GLChar::MsgReqSlotToHold ( NET_MSG_GENERIC* nmg )
{
	...
	/*dmk14 vehicle disable release when active*/
	if( pNetMsg->emSlot == SLOT_VEHICLE && ( m_bVehicle ) )	return S_FALSE;
...
HRESULT GLChar::MsgReqHoldToSlot ( NET_MSG_GENERIC* nmg )
{
	...
	if( m_pLandMan->IsPartySparring() )
	{
		//if( pItem->sSuitOp.emSuit > SUIT_HANDHELD || pItem->sSuitOp.emSuit < SUIT_UPPER )		return S_FALSE;
	}
	const SITEMCUSTOM sItemCustom = GET_HOLD_ITEM ();
...
HRESULT GLChar::MsgReqBuyFromNpc ( NET_MSG_GENERIC* nmg )
{
	...
	{
		llPRICE2= pItem->sBasicOp.dwReqContributionPoint;
	}
	else
	{
		if( dwPrice == 0 )
	...
	if ( pItem->sBasicOp.dwReqContributionPoint != 0 )
	{
		if ( m_llContributionPoint < llPRICE3 )
		return E_FAIL;
	}
	...
	if ( pItem->ISPILE() )
	{
		...
		if ( pItem->sBasicOp.dwReqContributionPoint != 0 ) 
		{
			CheckMoneyUpdate( m_llContributionPoint, llPRICE3, FALSE, "Buy From Npc." );
			m_bMoneyUpdate = TRUE;
			m_llContributionPoint -= llPRICE3;	

			std::string strTIME = CDebugSet::GetCurTime ();
			CDebugSet::ToFile ( "ITEM_CONTRIBUTION.txt", "%s Buy Multiple Item [CharID: %d][CharName:%s][Contribution Spent:%I64d][Money Left:%I64d][ITEM MID:%d SID:%d][Number: %d] ",
			strTIME.c_str(), m_dwCharID, m_szName, llPRICE3, m_llContributionPoint, sDB_ITEM.sNativeID.wMainID, sDB_ITEM.sNativeID.wSubID,pNetMsg->wBuyNum );
		}
		else
		{
...
#endif
		{
			char szTempChar[256] = {0,};
			string str;
			sprintf( szTempChar, ",[%s][%s], Pile Buy Item From NPC %02d_%02d, %s x(%d/%d), GenNum(%d)", m_szUID, m_szName, 
									pSaleItem->sItemCustom.sNativeID.wMainID,
									pSaleItem->sItemCustom.sNativeID.wSubID,
									pItem->GetName(),
									pSaleItem->sItemCustom.wTurnNum,
									pItem->sDrugOp.wPileNum,
									pSaleItem->sItemCustom.lnGenNum);
			str = szTempChar;
	...
	else
	{
		...
		//	랜덤 옵션 발생.
		//
		if ( pitem_data_check->sBasicOp.emLevel >= LEVEL_RARE )
		...
		if ( pItem->sBasicOp.dwReqContributionPoint != 0 ) 
		{
			CheckMoneyUpdate( m_llContributionPoint, llPRICE2, FALSE, "Buy From Npc." );
			m_bMoneyUpdate = TRUE;
			m_llContributionPoint -= llPRICE2;	

			std::string strTIME = CDebugSet::GetCurTime ();
			CDebugSet::ToFile ( "ITEM_CONTRIBUTION.txt", "%s Buy Single Item [CharID: %d][CharName:%s][Contribution Spent:%I64d][Money Left:%I64d][ITEM MID:%d SID:%d] ",
			strTIME.c_str(), m_dwCharID, m_szName, llPRICE3, m_llContributionPoint, sITEM_NEW.sNativeID.wMainID, sITEM_NEW.sNativeID.wSubID );
		}
		else
		{
		...
		
		{
			char szTempChar[256] = {0,};
			string str;
			sprintf( szTempChar, ",[%s][%s], Buy Item From NPC %02d_%02d, %s x(%d/%d), GenNum(%d)", m_szUID, m_szName, 
									pSaleItem->sItemCustom.sNativeID.wMainID,
									pSaleItem->sItemCustom.sNativeID.wSubID,
									pItem->GetName(),
									pSaleItem->sItemCustom.wTurnNum,
...
*/
	...
	
	{
		char szTempChar[256] = {0,};
		string str;
		sprintf( szTempChar, ",[%s][%s], Sell Item To NPC %02d_%02d, %s x(%d/%d), GenNum(%d)", m_szUID, m_szName, 
								sHOLD_ITEM.sNativeID.wMainID,
								sHOLD_ITEM.sNativeID.wSubID,
								pItem->GetName(),
								sHOLD_ITEM.wTurnNum,
								pItem->sDrugOp.wPileNum,
								sHOLD_ITEM.lnGenNum);
...
HRESULT GLChar::MsgReqInvenDrug ( NET_MSG_GENERIC* nmg )
{
	...
	case ITEM_DRUG_CP:
		if ( m_sCombatPoint.dwNow == m_sCombatPoint.dwMax ) return E_FAIL;
...
HRESULT GLChar::MsgReqInvenBoxOpen ( NET_MSG_GENERIC* nmg )
{
	...
	// Cheat Protection - Box Level Required
	if ( m_wLevel < pItem->sBasicOp.wReqLevelDW )
	{
		return E_FAIL;
	}

	//Select Box
	if( pItem->sBox.m_bSelect )
	{
		WORD wSelect = 0;
		for ( int i=0; i<ITEM::SBOX::ITEM_SIZE; ++i )	if( pNetMsg->bSelect[i] ) wSelect++;
		if( pItem->sBox.m_nMaxSelect != wSelect )		return E_FAIL;
	}
	...
	for ( int i=0; i<ITEM::SBOX::ITEM_SIZE; ++i )
	{
		...
		//Select Box
		if( pItem->sBox.m_bSelect && !pNetMsg->bSelect[i] )			continue;
		//	Note : 아이템 생성.
		//
		CTime cTIME = CTime::GetCurrentTime();
		sITEM_NEW.tBORNTIME = cTIME.GetTime();
...
HRESULT GLChar::MsgReqInvenRandomBoxOpen ( NET_MSG_GENERIC* nmg )
{
	...
	if ( !pItem )
	{
		if (pItem->sBasicOp.emItemType != ITEM_RANDOMITEM ||
			pItem->sBasicOp.emItemType != ITEM_MYSTERY_BOX )
		{
			NetMsgFB.emFB = EMINVEN_RANDOMBOXOPEN_FB_FAIL;
			GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
			return E_FAIL;
	...
	//Mystery Box
	bool isMysteryBox = pItem->sBasicOp.emItemType == ITEM_MYSTERY_BOX;
	if (isMysteryBox)
	{
		if (pInvenItem->sItemCustom.isBoxLock)
		{
			NetMsgFB.emFB = EMINVEN_RANDOMBOXOPEN_FB_LOCK;
			GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
			return E_FAIL;
		}
	}
...
HRESULT GLChar::MsgReqInvenGrinding ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pItem->sSuitOp.gdDamage == GLDWDATA(0,0) )
	{
	...
	if ( bSucceeded )
	{
		strTemp.Format( ",[%s][%s], Upgrade Item Success %s [%02d_%02d] [+%d] -> [+%d], using %s [%02d_%02d]",m_szUID, m_szName, pItem->GetName(), pItem->sBasicOp.sNativeID.wMainID,pItem->sBasicOp.sNativeID.wSubID, cGrade, pInvenItem->sItemCustom.GETGRADE(pHold->sGrindingOp.emTYPE),
			pHold->GetName(), pHold->sBasicOp.sNativeID.wMainID,pHold->sBasicOp.sNativeID.wSubID );
	}
	else
	{
		strTemp.Format( ",[%s][%s], Upgrade Item Failed %s [%02d_%02d] [+%d] -> [+%d], using %s [%02d_%02d]",m_szUID, m_szName, pItem->GetName(), pItem->sBasicOp.sNativeID.wMainID,pItem->sBasicOp.sNativeID.wSubID, cGrade, pInvenItem->sItemCustom.GETGRADE(pHold->sGrindingOp.emTYPE),
			pHold->GetName(), pHold->sBasicOp.sNativeID.wMainID,pHold->sBasicOp.sNativeID.wSubID );
	}
...
HRESULT GLChar::MsgReqInvenRemodelOpen ( NET_MSG_GENERIC* nmg )
{
	...
	/*dmk14 reform*/
	m_sItemRemodel = pINVENITEM->sItemCustom.sNativeID;
...
HRESULT GLChar::MsgReqInvenRename ( NET_MSG_GENERIC* nmg )
{
	...
	if ( m_bEnableInventoryLock || m_bEnableStorageLock || m_bEnableEquipmentLock )
	{
		MsgFB.emFB = EMINVEN_RENAME_FB_FAIL;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}
...
HRESULT GLChar::MsgReqRebuildResult( NET_MSG_GENERIC* nmg )	// ITEMREBUILD_MARK
{
	...
	case EMREBUILD_RESULT_SUCCESS:	
		{
			if( !m_bUseRebuildCard || m_bIsReformProcess )		
			{
				return E_FAIL;
			}

			m_bIsReformProcess = true;
			...
			SITEMCUSTOM sSealedCustom = GET_SEALED_ITEM();
			SITEM* sItemSealed = GLItemMan::GetInstance().GetItem( sSealedCustom.sNativeID );
			std::vector<EMRANDOM_OPT> vecLockOpt;
			if( sItemSealed )
			{
				for( int i=0;i<SRANDOM_GEN::MAX_OPT;i++)	
				{
					if( pNetMsg->emLockOpt[i] != EMR_OPT_NULL )	
					{
						vecLockOpt.push_back( pNetMsg->emLockOpt[i] );
					}
				}

				if( vecLockOpt.size() > sItemSealed->sSuitOp.wReModelNum )		return E_FAIL;
			}

			SINVENITEM* pItemRemodel = m_cInventory.FindItem( ITEM_REMODEL, m_sItemRemodel );
			GLMSG::SNET_INVEN_REMODELOPEN_FB MsgFB;
			if( !pItemRemodel )
			{
				MsgFB.emFB = EMREQ_REMODELOPEN_FB_NOITEM;
			...
			//sealed
			SINVENITEM* pItemSealed = m_cInventory.FindItem( ITEM_SEALED_CARD, m_sItemSealed );
			if( !pItemSealed && vecLockOpt.size() > 0)
			{
				MsgFB.emFB = EMREQ_REMODELOPEN_FB_NOSEALEDITEM;
				GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);

				return E_FAIL;
			}

			if ( CheckCoolTime( pItemRemodel->sItemCustom.sNativeID ) )	return E_FAIL;

			if ( pItem->sRandomOpt.bUnableRandomCard )	return E_FAIL;

			DoDrugInvenItem ( pItemRemodel->wPosX, pItemRemodel->wPosY );
			if( vecLockOpt.size() > 0 ) DoDrugInvenItem ( pItemSealed->wPosX, pItemSealed->wPosY );
			...
			else
			{
				...
				if( pInvenItem )
				{
					m_sItemPrevCopy = pInvenItem->sItemCustom;
					//sealed
					if( vecLockOpt.size() > 0 )
						pInvenItem->sItemCustom.GENERATE_RANDOM_OPT( vecLockOpt );
					else
					...
					// 개조 성공했다
					if( vecLockOpt.size() > 0 )
						pNetMsg->emResult = EMREBUILD_RESULT_SUCCESS_WITH_STAMPCARD;
					else 
						pNetMsg->emResult = EMREBUILD_RESULT_SUCCESS;

					pNetMsg->sItemCopy = m_sItemPrevCopy;
					GLGaeaServer::GetInstance().SENDTOCLIENT( m_dwClientID, pNetMsg );
				}
			}
	...
	case EMREBUILD_RESULT_PREVIOUS:
		{
			...
			if( sRebuildItem.sNativeID != m_sItemPrevCopy.sNativeID )		return E_FAIL;
...

			{
				...
				if( pInvenItem )
				{
					pInvenItem->sItemCustom = m_sItemPrevCopy;
					GLITEMLMT::GetInstance().ReqRandomItem( sRebuildItem );
					...
					pNetMsg->emResult = EMREBUILD_RESULT_PREVIOUS;
					GLGaeaServer::GetInstance().SENDTOCLIENT( m_dwClientID, pNetMsg );
...
HRESULT GLChar::MsgReqGarbageResult( NET_MSG_GENERIC* nmg )	// ITEMREBUILD_MARK
{
	...
	//if( m_ItemGarbagePosX < EM_INVENSIZE_X && m_ItemGarbagePosY < EM_INVENSIZE_Y )
	{
...

		{
			char szTempChar[256] = {0,};
			string str;
			sprintf( szTempChar, ",[%s][%s], Deleted Item %02d_%02d, %s x(%d/%d), GenNum(%d)", m_szUID, m_szName, 
									pINVENITEM_DEL->sItemCustom.sNativeID.wMainID,
									pINVENITEM_DEL->sItemCustom.sNativeID.wSubID,
									pItem->GetName(),
									pINVENITEM_DEL->sItemCustom.wTurnNum,
									pItem->sDrugOp.wPileNum,
									pINVENITEM_DEL->sItemCustom.lnGenNum);
			str = szTempChar;
...
/*dmk14 reform*/
HRESULT GLChar::MsgReqRebuildMoveItemSealed( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNET_REBUILD_MOVE_ITEMSEALED* pNetMsg = (GLMSG::SNET_REBUILD_MOVE_ITEMSEALED*)nmg;
	GLMSG::SNET_REBUILD_MOVE_ITEMSEALED NetMsg;

	m_sRebuildItemSealed = NATIVEID_NULL();
	m_sItemSealed = NATIVEID_NULL();

	if( pNetMsg->sItemSeal != NATIVEID_NULL() )
	{
		SINVENITEM* pResistItem = m_cInventory.FindItem( ITEM_SEALED_CARD, pNetMsg->sItemSeal );
		if( pResistItem )
		{
			SITEM* pItem = GLItemMan::GetInstance().GetItem( pResistItem->sItemCustom.sNativeID );
			if( pItem && pItem->sBasicOp.emItemType == ITEM_SEALED_CARD )
			{
				NetMsg.sItemSeal = pNetMsg->sItemSeal;

				m_sItemSealed = pResistItem->sItemCustom.sNativeID;
				m_sRebuildItemSealed = pResistItem->sItemCustom.sNativeID;
			}
		}
	}
	GLGaeaServer::GetInstance().SENDTOCLIENT( m_dwClientID, &NetMsg );
...
HRESULT GLChar::MsgReqCardContribution ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pITEM->sBasicOp.emItemType == ITEM_CONTRIBUTION_CARD )
	{
		...
		m_llContributionPoint += dwPointReward;
...
HRESULT GLChar::MsgReqCardEPoint ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPC_CARD_EPOINT *pNetMsg = (GLMSG::SNETPC_CARD_EPOINT *)nmg;
	GLMSG::SNETPC_CARD_EPOINT_FB MsgFB;

	//cant find the item!
	SINVENITEM* pINVENITEM = m_cInventory.GetItem ( pNetMsg->wPosX, pNetMsg->wPosY );
	if ( !pINVENITEM )
	{
		MsgFB.emFB = EMINVEN_CARD_EPOINT_FB_NOITEMINVEN;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}
	...
	if ( CheckCoolTime( pINVENITEM->sItemCustom.sNativeID ) )
	{
		MsgFB.emFB = EMINVEN_CARD_EPOINT_FB_FAIL;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}
	...
	if ( !pITEM )
	{
		MsgFB.emFB = EMINVEN_CARD_EPOINT_FB_NOITEMDATA;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}

	//wrong item type
	if ( pITEM->sBasicOp.emItemType != ITEM_EPOINT_CARD )		
	{
		MsgFB.emFB = EMINVEN_CARD_EPOINT_FB_NOITEMTYPE;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}
	...
	if ( pITEM->sBasicOp.emItemType == ITEM_EPOINT_CARD )
	{
		...
		CUpdateMoneyPoints* pDbAction = new CUpdateMoneyPoints ( dwPointReward, m_szUID );
		GLDBMan* pDBMan = GLGaeaServer::GetInstance().GetDBMan ();
		if ( pDBMan ) pDBMan->AddJob ( pDbAction );
	}
	else
	...
	MsgFB.emFB = EMINVEN_CARD_EPOINT_FB_CLOSE;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);

	DoDrugInvenItem ( pNetMsg->wPosX, pNetMsg->wPosY );

	MsgFB.emFB = EMINVEN_CARD_EPOINT_FB_OK;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);

	return S_OK;
}

HRESULT GLChar::MsgReqResetBrightPoint ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPC_CARD_BRIGHT *pNetMsg = (GLMSG::SNETPC_CARD_BRIGHT *)nmg;
	GLMSG::SNETPC_CARD_BRIGHT_FB MsgFB;

	//cant find the item!
	SINVENITEM* pINVENITEM = m_cInventory.GetItem ( pNetMsg->wPosX, pNetMsg->wPosY );
	if ( !pINVENITEM )
	{
		MsgFB.emFB = EMINVEN_CARD_BRIGHT_FB_NOITEMINVEN;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}
	...
	if ( CheckCoolTime( pINVENITEM->sItemCustom.sNativeID ) )
	{
		MsgFB.emFB = EMINVEN_CARD_BRIGHT_FB_FAIL;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}
	...
	if ( !pITEM )
	{
		MsgFB.emFB = EMINVEN_CARD_BRIGHT_FB_NOITEMDATA;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}

	//wrong item type
	if ( pITEM->sBasicOp.emItemType != ITEM_RESET_BRIGHT_CARD )		
	{
		MsgFB.emFB = EMINVEN_CARD_BRIGHT_FB_NOITEMTYPE;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}
	...
	if ( pITEM->sBasicOp.emItemType == ITEM_RESET_BRIGHT_CARD )
	{
		DWORD dwBrightReset = pITEM->sDrugOp.wCureVolume;

		m_nBright = dwBrightReset;
	...
	MsgFB.emFB = EMINVEN_CARD_BRIGHT_FB_CLOSE;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
	...
	MsgFB.emFB = EMINVEN_CARD_BRIGHT_FB_OK;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);

	return S_OK;
}

HRESULT GLChar::MsgMysteryBox(NET_MSG_GENERIC* nmg)
{
	GLMSG::SNET_MYSTERY_BOX *pNetMsg = (GLMSG::SNET_MYSTERY_BOX *)nmg;
	...
	if ( pHold->sBasicOp.emItemType != ITEM_MYSTERY_KEY )	return S_FALSE;
	...
	if (pItem->sBasicOp.emItemType != ITEM_MYSTERY_BOX) return S_FALSE;
	
	//key checker (this is to enable multiple types of mystery box & key) //cndev
	if ( pHold->sBasicOp.sNativeID != pItem->sBox.sITEMS[0].nidITEM )	return S_FALSE;

	if (!pInvenItem->sItemCustom.isBoxLock) return S_FALSE;

	pInvenItem->sItemCustom.isBoxLock = false;
	...
	GLMSG::SNET_MYSTERY_BOX_FB NetMsgFB;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
...
HRESULT GLChar::MsgReqReformWeapon ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNET_INVEN_REFORM_WEAPON *pNetMsg = (GLMSG::SNET_INVEN_REFORM_WEAPON *)nmg;
	...
	if ( pHold->sBasicOp.emItemType != ITEM_SKILL_REFORMCARD )	return S_FALSE;
	...
	if ( pItem->sBasicOp.emItemType != ITEM_SUIT ) return S_FALSE;

	if ( pItem->sSuitOp.emSuit != SUIT_HANDHELD) return S_FALSE;

	if ( !pItem->sBasicOp.isHaveSkill() ) return S_FALSE;

	float fNowRate = seqrandom::getpercent();
	float fLOW = 0;
	GLWEAPON_LINKSKILL glSkillGet;
	...
	for(; i < j; ++i )
	{
		...
		if ( fLOW <= fNowRate && fNowRate < (fLOW+glSkill.fPossibility) )
		{
			glSkillGet = glSkill;
			break;
		}

		fLOW += glSkill.fPossibility;
	}
	
	bool bSuccess = glSkillGet.sNativeID.IsValidNativeID();
	if(bSuccess) pInvenItem->sItemCustom.sSkillLinkID = glSkillGet.sNativeID;
	...
	GLMSG::SNET_INVEN_REFORM_WEAPON_FB NetMsgFB;
	NetMsgFB.bSuccess = bSuccess;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
...
HRESULT GLChar::MsgReqRegisterCodex ( NET_MSG_GENERIC* nmg )
{
	...
	BYTE cGRADE = pInvenItem->sItemCustom.GETGRADE ( EMGRINDING_NONE );
	...
	{
		NetMsgFB.emFB = EMREQ_REGISTER_CODEX_FB_INVALIDITEM;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
		return E_FAIL;
	}
