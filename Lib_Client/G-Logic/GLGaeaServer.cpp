...
HRESULT GLGaeaServer::Create ( DWORD dwMaxClient, DxMsgServer *pMsgServer, DxConsoleMsg* pConsoleMsg, GLDBMan* pDBMan, int nServiceProvider, const char* szMapList, DWORD dwFieldSID, int nChannel, BOOL bUseIntelTBB )
{
	...
	for ( ; iter!=iter_end; ++iter )
	{
		...
		//pNewLandMan->SetPVPItemOnly ( pMapNode->bPVPItemOnly );
...
BOOL GLGaeaServer::DropOutPET ( DWORD dwGUID, bool bLeaveFieldServer, bool bMoveMap )
{
	...
	{
		//CDebugSet::ToLogFile ( "ERROR : DropOutPET() in GLGaeaServer::DropOutPET(), PetGUID : %u", dwGUID );
		return FALSE;
	}
...
HRESULT GLGaeaServer::FrameMove ( float fTime, float fElapsedTime )
{
	...
	if( m_wSlotMin != 0 && m_wSlotMax !=0 && m_sItemSlotReward != NATIVEID_NULL() )
	{
		srand(time(NULL));
		WORD range = (m_wSlotMax-m_wSlotMin)+1;
		m_wWinner = rand()%range+m_wSlotMin;
	}
...
/*void GLGaeaServer::GerberHackShieldHeartbeat( DWORD dwClientID, DWORD dwGaeaID )
{
	PGLCHAR pChar = GetChar ( dwGaeaID );
	if ( !pChar ) return;
}  */
...
BOOL GLGaeaServer::UpdatePartyList ( PARTYLINKDATA &sPartyNew )
{
	GLMSG::SNETPC_REQ_PARTY_SEARCH_UPDATE NetMsg;
	NetMsg.sParty	= sPartyNew;
	SENDTOAGENT(&NetMsg);

	return TRUE;
}

BOOL GLGaeaServer::RemoveFromPartyList ( PARTYLINKDATA sParty, DWORD dwPartyID, BOOL bFromExitGame  )
{
	if ( bFromExitGame && dwPartyID != PARTY_NULL )
	{
		PARTYLINKDATA sDATAOld;
		sDATAOld.dwPartyID = dwPartyID;

		GLMSG::SNETPC_REQ_PARTY_SEARCH_UPDATE NetMsg;
		NetMsg.sParty	= sDATAOld;
		NetMsg.bDelete	= TRUE;
		SENDTOAGENT(&NetMsg);
