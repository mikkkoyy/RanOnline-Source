...
namespace GLMSG
{
	...
	struct SNET_GM_CHAR_INFO_FLD_FB
	{
		...
		GLDWDATA			sSP;
		GLLLDATA			sEXP;
		GLDWDATA			sCP; /*combatpoint logic, Juver, 2017/05/29 */
		char				szCLUB[CHAR_SZNAME];
	...
	struct SNET_SLOT_EVENT_GEN_BRD
	{
		NET_MSG_GENERIC		nmg;
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_SLOT_EVENT_GEN_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_SLOT_EVENT_GEN_FLD
	{
		NET_MSG_GENERIC		nmg;
		...
		SNET_SLOT_EVENT_GEN_FLD ()
			...
			, bRoll(false)
		{
			nmg.dwSize = sizeof(SNET_SLOT_EVENT_GEN_FLD);
			nmg.nType = NET_MSG_GCTRL_SLOT_EVENT_GEN_FLD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
