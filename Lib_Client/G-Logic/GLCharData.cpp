...
void SCHARDATA2::Assign ( SCHARDATA2 &sDATA )
{
	...
	m_mapPandoraResults = sDATA.m_mapPandoraResults;
}
...
/*dmk14 reform*/
const SITEMCUSTOM& SCHARDATA2::GET_SEALED_ITEM()	// ITEMREBUILD_MARK
{
	static SITEMCUSTOM sItemCustom;
	sItemCustom.sNativeID = NATIVEID_NULL();

	if( m_sRebuildItemSealed == NATIVEID_NULL() )
		return sItemCustom;

	SINVENITEM* pResistItem = m_cInventory.FindItem( ITEM_SEALED_CARD, m_sRebuildItemSealed );
	if( !pResistItem )
		return sItemCustom;
