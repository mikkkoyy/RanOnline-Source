...
HRESULT GLChar::MsgPMarketReqItem ( NET_MSG_GENERIC* nmg )
{
	...
	// add item price validation
	if ( pNetMsg->llMoney < 0 )
	{
		NetMsgFB.emFB = EMPMARKET_REGITEM_FB_FAIL;
		GLGaeaServer::GetInstance().SENDTOCLIENT ( m_dwClientID, (NET_MSG_GENERIC*) &NetMsgFB );
		CDebugSet::ToFileWithTime( "MsgPMarketReqItem.txt","[%s]-[%d][%s] tried to input invalid price value of %I64d",m_szUID,GetCharID(),m_szName,(LONGLONG)pNetMsg->llMoney);
		return S_FALSE;
	}
...
*/
	...
	//CDebugSet::ToFileWithTime("PrivateMarketLogs.csv",strTemp);
	CString strName;
	...
	/*dmk14 ingame web*/
	NetMsgM1.dwPremium = m_dwPremiumPoints;
	NetMsgM1.dwCombat = m_dwCombatPoints;
	...
	/*dmk14 ingame web*/
	NetMsgM2.dwPremium = pCHAR->m_dwPremiumPoints;
	NetMsgM2.dwCombat = pCHAR->m_dwCombatPoints;
