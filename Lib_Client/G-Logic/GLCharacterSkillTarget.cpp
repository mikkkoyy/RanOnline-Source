...
BOOL GLCharacter::SKT_TARZONEENERMY ( const STARGETID &sTargetID, const D3DXVECTOR3 *pvTarPos, const GLSKILL &sSKILL, const WORD wLEVEL )
{
	...
	{
		PGLSKILL pSkillL = GLSkillMan::GetInstance().GetData ( sSKILL.m_sEXT_DATA.sADDSKILL );
		if ( !pSkillL || pSkillL && pSkillL->m_sBASIC.emROLE == SKILL::EMROLE_PASSIVE ) return TRUE;
		if( pSkillL->m_sBASIC.emIMPACT_SIDE != SIDE_OUR ) return TRUE;

		const SKILL::CDATA_LVL &sSKILL_DATA = pSkillL->m_sAPPLY.sDATA_LVL[sSKILL.m_sEXT_DATA.dwADDSKILLLEVEL];
		WORD wTARNUM = sSKILL_DATA.wTARNUM;
...
BOOL GLCharacter::SelectSkillTarget ( const STARGETID &sTargetID, const D3DXVECTOR3 *pvTarPos )
{
	...
	case TAR_ZONE:
		...
		return FALSE;
	case TAR_SPECIFIC: //Teleport Skill Effect
		{
			if(pSkill->m_sBASIC.emIMPACT_SIDE != SIDE_OUR)	return FALSE;
			if ( m_bRunBot )	return FALSE;

			switch (pSkill->m_sBASIC.emIMPACT_REALM)
			{
			case REALM_ZONE:
				{
					GLPARTY_CLIENT* pParty = GLPartyClient::GetInstance().FindMember ( GetCtrlID() );

					if ( pParty ) SKT_TARZONEOUR ( sTargetID, pvTarPos, *pSkill, sSkill.wLevel );
