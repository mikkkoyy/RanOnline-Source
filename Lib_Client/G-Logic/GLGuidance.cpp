...
/////////////////////////////////////////////////////////////////////////////
// LG-7 CWRanking
BOOL SortByKill(const SCW_RANKING& L_Rank, const SCW_RANKING& R_Rank)
{
...
GLGuidance& GLGuidance::operator= ( const GLGuidance& value )
{
	...
	/////////////////////////////////////////////////////////////////////////////
	// LG-7 CWRanking
	m_mapCWRanking		= value.m_mapCWRanking;
	m_vecCWRanking		= value.m_vecCWRanking;
...
bool GLGuidance::Load ( std::string strFile )
{
	...
	/////////////////////////////////////////////////////////////////////////////
	// LG-7 CWRanking
	cFILE.getflag("GUIDANCE", "AWARD_ITEM_1", 0, 2, m_sCWRankingReward.sReward_[0].wMainID);
	cFILE.getflag("GUIDANCE", "AWARD_ITEM_1", 1, 2, m_sCWRankingReward.sReward_[0].wSubID);

	cFILE.getflag("GUIDANCE", "AWARD_ITEM_2", 0, 2, m_sCWRankingReward.sReward_[1].wMainID);
	cFILE.getflag("GUIDANCE", "AWARD_ITEM_2", 1, 2, m_sCWRankingReward.sReward_[1].wSubID);

	cFILE.getflag("GUIDANCE", "AWARD_ITEM_3", 0, 2, m_sCWRankingReward.sReward_[2].wMainID);
	cFILE.getflag("GUIDANCE", "AWARD_ITEM_3", 1, 2, m_sCWRankingReward.sReward_[2].wSubID);

	cFILE.getflag("GUIDANCE", "AWARD_ITEM_4", 0, 2, m_sCWRankingReward.sReward_[3].wMainID);
	cFILE.getflag("GUIDANCE", "AWARD_ITEM_4", 1, 2, m_sCWRankingReward.sReward_[3].wSubID);

	cFILE.getflag("GUIDANCE", "AWARD_ITEM_5", 0, 2, m_sCWRankingReward.sReward_[4].wMainID);
	cFILE.getflag("GUIDANCE", "AWARD_ITEM_5", 1, 2, m_sCWRankingReward.sReward_[4].wSubID);

	cFILE.getflag("GUIDANCE", "AWARD_ITEM_6", 0, 2, m_sCWRankingReward.sReward_[5].wMainID);
	cFILE.getflag("GUIDANCE", "AWARD_ITEM_6", 1, 2, m_sCWRankingReward.sReward_[5].wSubID);

	cFILE.getflag("GUIDANCE", "AWARD_ITEM_7", 0, 2, m_sCWRankingReward.sReward_[6].wMainID);
	cFILE.getflag("GUIDANCE", "AWARD_ITEM_7", 1, 2, m_sCWRankingReward.sReward_[6].wSubID);

	cFILE.getflag("GUIDANCE", "AWARD_ITEM_8", 0, 2, m_sCWRankingReward.sReward_[7].wMainID);
	cFILE.getflag("GUIDANCE", "AWARD_ITEM_8", 1, 2, m_sCWRankingReward.sReward_[7].wSubID);

	cFILE.getflag("GUIDANCE", "AWARD_ITEM_9", 0, 2, m_sCWRankingReward.sReward_[8].wMainID);
	cFILE.getflag("GUIDANCE", "AWARD_ITEM_9", 1, 2, m_sCWRankingReward.sReward_[8].wSubID);

	cFILE.getflag("GUIDANCE", "AWARD_ITEM_10", 0, 2, m_sCWRankingReward.sReward_[9].wMainID);
	cFILE.getflag("GUIDANCE", "AWARD_ITEM_10", 1, 2, m_sCWRankingReward.sReward_[9].wSubID);
	/////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
// LG-7 CWRanking
void GLGuidance::AddCWRanking(DWORD dwKillID, DWORD dwDeathID)
{
	MAP_CW_RANKING_ITER iter_find; 

	iter_find = m_mapCWRanking.find(dwKillID);
	if (iter_find != m_mapCWRanking.end())
	{
	...
	else
	{
		...
		if (pChar)
		{
			SCW_RANKING sCWRanking;
			sCWRanking.dwCharID = dwKillID;
			sCWRanking.wKill++;
			StringCchCopy(sCWRanking.szChaName, CHAR_SZNAME, pChar->m_szName);

			m_mapCWRanking.insert(make_pair(dwKillID, sCWRanking));
		}
	}

	iter_find = m_mapCWRanking.find(dwDeathID);
	if (iter_find != m_mapCWRanking.end())
	{
	...
	else
	{
		...
		{
			SCW_RANKING sCWRanking;
			sCWRanking.dwCharID	= dwDeathID;
			sCWRanking.wDead++;
			StringCchCopy(sCWRanking.szChaName, CHAR_SZNAME, pChar->m_szName);

			m_mapCWRanking.insert(make_pair(dwDeathID, sCWRanking));
		}
	}
}
void GLGuidance::UpdateCWRanking()
{
	m_vecCWRanking.clear();

	MAP_CW_RANKING_ITER iter_begin	= m_mapCWRanking.begin();
	MAP_CW_RANKING_ITER iter_end	= m_mapCWRanking.end();

	for (; iter_begin != iter_end; iter_begin++)
		m_vecCWRanking.push_back((*iter_begin).second);

	if (!m_vecCWRanking.empty())
	{
		sort(m_vecCWRanking.begin(), m_vecCWRanking.end(), SortByKill);

		for (size_t i = 0; i < m_vecCWRanking.size(); i++)
			m_vecCWRanking[i].nIndex = i;

		GLMSG::SNET_REQ_CW_RANKING_FB NetMsg;
		NetMsg.dwID = m_dwID;

		if (m_vecCWRanking.size() < 10)
		{
			for (size_t i = 0; i < m_vecCWRanking.size(); i++)
				NetMsg.sCWRanking_[i] = m_vecCWRanking[i];
		}
		else
		{
			for (int i = 0; i < 10; i++)
		...
		GLGaeaServer::GetInstance().SENDTOCLIENT_ONMAP(m_dwClubMap, &NetMsg);
		GLGaeaServer::GetInstance().SENDTOCLIENT_ONMAP(m_dwClubHallMap, &NetMsg);
	}
}
void GLGuidance::CWReward()
{
	if (!m_vecCWRanking.empty())
	{
		if (m_vecCWRanking.size() < 10)
		{
			for (size_t i = 0; i < m_vecCWRanking.size(); i++)
			{
				PGLCHAR pChar = GLGaeaServer::GetInstance().GetCharID(m_vecCWRanking[i].dwCharID);
				if (pChar)
				{
					SITEM* pItem = GLItemMan::GetInstance().GetItem(m_sCWRankingReward.sReward_[i]);
					if (pItem)
					{
...
void GLGuidance::ReqCWRanking(DWORD dwClientID)
{
	if (m_vecCWRanking.empty()) return;
...
void GLGuidance::ReqMyCWRanking(DWORD dwClientID, DWORD dwCharID)
{
	if (m_vecCWRanking.empty()) return;

	VEC_CW_RANKING_ITER iter_begin	= m_vecCWRanking.begin();
	VEC_CW_RANKING_ITER iter_end	= m_vecCWRanking.end();

	for (; iter_begin != iter_end; iter_begin++)
	{
		if (dwCharID == (*iter_begin).dwCharID)
		{
			GLMSG::SNET_REQ_MY_CW_RANKING_FB NetMsg;
			NetMsg.sCWRanking = (*iter_begin);
			GLGaeaServer::GetInstance().SENDTOCLIENT(dwClientID, &NetMsg);
			break;
		}
	}
...
bool GLGuidanceFieldMan::BeginBattle ( DWORD dwID )
{
	...
	/////////////////////////////////////////////////////////////////////////////
	// LG-7 CWRanking
	pGuid->m_mapCWRanking.clear();
	pGuid->m_vecCWRanking.clear();
	/////////////////////////////////////////////////////////////////////////////
...
bool GLGuidanceFieldMan::FrameMove ( float fElaps )
{
	for ( GUIDANCE_VEC::size_type i=0; i<m_vecGuidance.size(); ++i )
	{
		...
		if (sGuidance.IsBattle())
		{
			...
			if (sGuidance.m_fCWRankingUpdate > 60.0f)
			{
				sGuidance.m_fCWRankingUpdate = 0.0f;
				sGuidance.UpdateCWRanking();
			}
		}
		/////////////////////////////////////////////////////////////////////////////
	}
