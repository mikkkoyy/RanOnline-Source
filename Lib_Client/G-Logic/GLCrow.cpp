...
HRESULT GLCrow::MoveTo ( const D3DXVECTOR3 &vTarPos, bool bJump )
{
...
bool GLCrow::MoveTo ( const D3DXVECTOR3 *vTarPos, bool bJump, DWORD dwFlags )
{
	if ( bJump ) 
	{
...
HRESULT GLCrow::FrameMove ( float fTime, float fElapsedTime )
{
	...
	if ( bCHECK_TAR && m_TargetID.emCrow==CROW_PC )
	{
		...
		if ( pCHAR )
		{
			...
			if( !(pLevelEtcFunc && pLevelEtcFunc->m_bUseFunction[EMETCFUNC_MOBCHASE]) || !bGroupTarget )
			{
				//	Note : 과도한 몹몰이 방지.
				//
				// 뛰기가능한 몹인가?
	...
	if ( !bCHECK_TAR || !bVALID_TAR )
	{
		...
		if( IsGroupLeader() && m_pLandMan->GET_GROUPTARGET( GetGroupName() ).dwID != EMTARGET_NULL )
		{
			...
			for( ; setMember_iter != setGroupMember.end(); ++setMember_iter )
			{
				...
				if( pGLCrow->m_TargetID == m_pLandMan->GET_GROUPTARGET( GetGroupName() ) )
					pGLCrow->ReSetFLAG(EMTARGET);
			}

			m_pLandMan->RESET_GROUPTARGET( GetGroupName() );
		}
		/*
		if( IsGroupMember() )
		{
...
DWORD GLCrow::ReceiveDamage ( const EMCROW emACrow, const DWORD dwAID, const DWORD dwDamage, const BOOL bShock )
{
	...
	if ( m_dwNowHP > 0 )
	{
		...
		if ( emACrow == CROW_PC && m_pCrowData && m_pCrowData->m_emNPCType == EMNPC_TYPE_EVENT_TOWER )
		{
			...
			if ( _pchar )
			{
				if( _pchar->m_wSchool != m_wTyrannyOwner )	m_dwTyrannyDamage[_pchar->m_wSchool] += dwDxHp;
				switch( _pchar->m_wSchool )
				{
				case 0:
					{
						if( m_dwTyrannyDamage[1] >= dwDxHp && m_wTyrannyOwner != 1 ) m_dwTyrannyDamage[1] -= dwDxHp;
						if( m_dwTyrannyDamage[2] >= dwDxHp && m_wTyrannyOwner != 2 ) m_dwTyrannyDamage[2] -= dwDxHp;
					}
					break;
				case 1:
					{
						if( m_dwTyrannyDamage[0] >= dwDxHp && m_wTyrannyOwner != 0 ) m_dwTyrannyDamage[0] -= dwDxHp;
						if( m_dwTyrannyDamage[2] >= dwDxHp && m_wTyrannyOwner != 2 ) m_dwTyrannyDamage[2] -= dwDxHp;
					}
					break;
				case 2:
		...
		if ( emACrow == CROW_SUMMON && m_pCrowData && m_pCrowData->m_emNPCType == EMNPC_TYPE_EVENT_TOWER )
		{
			...
			if ( psummon_field && psummon_field->m_pOwner )
			{
				...
				{
					if( psummon_field->m_pOwner->m_wSchool != m_wTyrannyOwner )	m_dwTyrannyDamage[psummon_field->m_pOwner->m_wSchool] += dwDxHp;
					switch( psummon_field->m_pOwner->m_wSchool )
					{
					case 0:
						{
							if( m_dwTyrannyDamage[1] >= dwDxHp && m_wTyrannyOwner != 1 ) m_dwTyrannyDamage[1] -= dwDxHp;
...
void GLCrow::ReceiveTeleport ( const D3DXVECTOR3 &vMovePos )
{
	if ( !IsValidBody() )	return;
