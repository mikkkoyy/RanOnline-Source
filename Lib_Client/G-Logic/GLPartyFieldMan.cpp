...
BOOL GLPartyFieldMan::MsgPartyFNew ( NET_MSG_GENERIC* nmg )
{
	...
	for ( DWORD i=0; i<pNetMsg->dwPartyNum; ++i )
	{
		...
		NetMsg.m_wLevel = pChar->m_wLevel;
	...
	/*Party link*/
	PGLCHAR pCharData = GLGaeaServer::GetInstance().GetChar ( pParty->m_dwMASTER );
	PARTYLINKDATA sDATA;

	if ( pCharData )
	{
		sDATA.m_emClass = pCharData->m_emClass;
		sDATA.m_sMapID = pCharData->m_sMapID;
		sDATA.m_wSchool = pCharData->m_wSchool;
		sDATA.sOption = pParty->m_sOPTION;
		sDATA.dwPartyID = pCharData->m_dwPartyID;
		sDATA.m_dwGaeaID = pCharData->m_dwGaeaID;
		sDATA.m_wMembers = (WORD)pParty->GETNUMBER();
		StringCchCopy ( sDATA.m_szName, CHAR_SZNAME, pCharData->m_szName );
		GLGaeaServer::GetInstance().UpdatePartyList( sDATA ); // AddToPartyList
	}
...
BOOL GLPartyFieldMan::MsgPartyAdd ( NET_MSG_GENERIC* nmg )
{
	...
	if (pCharData )
	{
		...
		sDATA.m_wMembers = (WORD)pParty->GETNUMBER();
		StringCchCopy ( sDATA.m_szName, CHAR_SZNAME, pCharData->m_szName );
		GLGaeaServer::GetInstance().UpdatePartyList ( sDATA );
	}
...
BOOL GLPartyFieldMan::MsgPartyDissolve ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pCharData )
	{	
		//for Dissolve ingame or leave if 2 members left.
		sDATA.m_emClass = pCharData->m_emClass;
		sDATA.m_sMapID = pCharData->m_sMapID;
		sDATA.m_wSchool = pCharData->m_wSchool;
		sDATA.sOption = pParty->m_sOPTION;
		...
		GLGaeaServer::GetInstance().RemoveFromPartyList( sDATA );
	}
	else
	{
		//if Players or members leave out of the game.
		sDATA.dwPartyID = pNetMsg->dwPartyID;
		GLGaeaServer::GetInstance().RemoveFromPartyList( sDATA, pNetMsg->dwPartyID, TRUE );
	}
...
BOOL GLPartyFieldMan::MsgPartyAuthority( NET_MSG_GENERIC* nmg )
{
	...
	{
		PARTYLINKDATA sDATA;
		sDATA.m_emClass = pNewMaster->m_emClass;
		sDATA.m_sMapID = pNewMaster->m_sMapID;
		sDATA.m_wSchool = pNewMaster->m_wSchool;
		sDATA.sOption = pParty->m_sOPTION;
		sDATA.dwPartyID = pParty->m_dwPARTYID;
		sDATA.m_dwGaeaID = pNewMaster->m_dwGaeaID;
		sDATA.m_wMembers = (WORD)pParty->GETNUMBER();
		StringCchCopy ( sDATA.m_szName, CHAR_SZNAME, pNewMaster->m_szName );
		GLGaeaServer::GetInstance().UpdatePartyList ( sDATA );
	}
