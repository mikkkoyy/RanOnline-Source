...
DWORD GLLandMan::DropCrow ( GLMobSchedule *pMobSch )
{
	...
	pNewCrow->m_dwGlobalID = dwGlobID;
		...
		pNewCrow->m_pCrowData->m_dwNotice&EMCROW_NOTICE_RESPAWN2 )
	{
		...
		{
			CString strTEXT;
			strTEXT.Format ( ID2SERVERTEXT( "CROW_RESPAWN" ), pNewCrow->m_pCrowData->GetName(), szMAP_NAME );
...
void GLLandMan::DoSkillActEx ( const SSKILLACTEX &sACTEX )
{
	...
	if ( pSkill != NULL )
	{
		DAMAGE_SPEC	sDamageSpec = pREACTOR->GetDamageSpec();
	...
	if ( sACTEX.bTeleport )
	{
		TeleportAct( sACTEX.sID, sACTEX.vTeleport_Pos);
	}
	...
	//if ( spNavi )
	{
		...
		if( bCoA )
		{
			...
			{							
				...
				if ( !IsLayerSimpleCollision( vPosA, vPosB ) )
				{
					GetNavi()->IsCollision ( D3DXVECTOR3(vPosB.x,vPosB.y+5.0f,vPosB.z), D3DXVECTOR3(vPosB.x,vPosB.y-5.0f,vPosB.z), vDestPos, dwCoB, bCoB );	
					if ( bCoB )
					{
						if ( GetNavi()->LineOfSightTest ( dwCoA, vCoA, dwCoB, vDestPos ) )
						{
							return true;
						}
					}
						   ...
						   DWORD				dwSwapType )
{
	...
	{
		pActor->ReceivePushPull ( vReactorPos, fPUSH_SPEED, false, false, true, AN_SUB_02 );
		pActor->SetPosition( vReactorPos );
	} //IF

	if( dwSwapType == 0 || dwSwapType == 2 )			//reactor
	{
		pReactor->ReceivePushPull ( vActorPos, fPUSH_SPEED, false, false, true, AN_SUB_02 );
...
void GLLandMan::TeleportAct( const STARGETID dwActorID, D3DXVECTOR3 vPos) 
{
	GLACTOR* pACTOR = GLGaeaServer::GetInstance().GetTarget ( this, dwActorID );
	if ( !pACTOR )	
		return;

	pACTOR->ReceiveTeleport(vPos);
}

/*anti shit gs*/
BOOL GLLandMan::IsLayerSimpleCollision( const D3DXVECTOR3& vStart, const D3DXVECTOR3& vEnd )
{
	/*dmk14 terrain check*/
	//if( !RANPARAM::bTerrainCheck )	return FALSE;

	D3DXVECTOR3 vREACTOR_POS = vEnd;
	D3DXVECTOR3 vDir = ( vREACTOR_POS - vStart );
			
	D3DXVECTOR3 vACTOR_POS = vStart;
	D3DXVECTOR3 vDirZ = ( vACTOR_POS - vEnd );
	vDir.y = 0.0f;
	D3DXVec3Normalize (  &vDir, &vDir );
	...
	if ( m_pNaviMesh )
	{
		...
		m_pNaviMesh->IsCollision ( D3DXVECTOR3(vPosA.x,vPosA.y+5.0f,vPosA.z), D3DXVECTOR3(vPosA.x,vPosA.y-5.0f,vPosA.z), vCoA, dwCoA, bCoA );
		if ( bCoA )
		{
			BOOL bCoB(FALSE);
			DWORD dwCoB(0);
			...
			m_pNaviMesh->IsCollision ( D3DXVECTOR3(vPosB.x,vPosB.y+5.0f,vPosB.z), D3DXVECTOR3(vPosB.x,vPosB.y-5.0f,vPosB.z), vCoB, dwCoB, bCoB );
			if ( !bCoB )																return TRUE;
			if ( bCoB && !m_pNaviMesh->LineOfSightTest ( dwCoA, vCoA, dwCoB, vCoB ) )	return TRUE;
		}
	}
