...
//		cFILE.getflag( "fDISPRICE", 1, 1, fDISPRICE );
        ...
        cFILE.getflag( "nidREBIRTH_RECOMMEND_ITEM", 1, 2, nidREBIRTH_RECOMMEND_ITEM.wMainID );
        cFILE.getflag( "nidREBIRTH_RECOMMEND_ITEM", 2, 2, nidREBIRTH_RECOMMEND_ITEM.wSubID );
        cFILE.getflag( "strREBIRTH_RECOMMEND_ITEM", 1, 1, strREBIRTH_RECOMMEND_ITEM );
        cFILE.getflag( "wAUTO_REBIRTH_TIMER", 1, 1, wAUTO_REBIRTH_TIMER );

        cFILE.getflag( "nidDIALOGUE_RECOMMEND_ITEM", 1, 2, nidDIALOGUE_RECOMMEND_ITEM.wMainID );
        cFILE.getflag( "nidDIALOGUE_RECOMMEND_ITEM", 2, 2, nidDIALOGUE_RECOMMEND_ITEM.wSubID );
        cFILE.getflag( "strDIALOGUE_RECOMMEND_ITEM", 1, 1, strDIALOGUE_RECOMMEND_ITEM );
		...
		cFILE.getflag( "wLEVEL_REQ_GAMETIME", 1, 1, wLEVEL_REQ_GAMETIME );
		...
		cFILE.getflag( "fMAX_RESIST", 1, 1, fMAX_RESIST );
		...
		for ( i = 0; i < SYSTEM_BUFF_SETTING_SIZE; ++i )
		{
			strTemp.Format("SYSTEMBUFF_%d", i );

            cFILE.getflag( strTemp.GetString(), 1, 3, sRandomSystemBuff[i].sSystemID.wMainID );
			cFILE.getflag( strTemp.GetString(), 2, 3, sRandomSystemBuff[i].sSystemID.wSubID);
			cFILE.getflag( strTemp.GetString(), 3, 3, sRandomSystemBuff[i].fRATE );
		}
		...
		cFILE.getflag( "wItemPass", 1, 1, wItemPass );
		...
		//PANDORA
		LOADFILE_PANDORA ( "pandorabox.ini" );

		LOAD_SKILLPREREQ ( "SkillLearnPrereq.lua" ); //add Skill Prerequisites 
		LOAD_WEAPON_LINKSKILL ( "ItemLinkSkill.lua" ); //Weapon Skill - SqueeeAK
		...
		// Item Information color.
        {
            ...
            int tempItemRank[LEVEL_NSIZE ][nRGB];
            ...
            {
                cFILE.getflag( "dwDefault", 1+i, 3, tempDefault[i] );
                for ( int j = 0; j < LEVEL_NSIZE; j++ )
                {
                    std::string strItemRank = sc::string::format("dwItemRank%d", j);
                    cFILE.getflag( strItemRank, 1+i, 3, tempItemRank[j][i] );
                }
                cFILE.getflag( "dwReqWrong", 1+i, 3, tempReqWrong[i] );
                cFILE.getflag( "dwInchant1", 1+i, 3, tempInchant1[i] );
                cFILE.getflag( "dwInchant2", 1+i, 3, tempInchant2[i] );
                cFILE.getflag( "dwEquipCostum", 1+i, 3, tempEquipCostum[i] );
                cFILE.getflag( "dwRankUp1", 1+i, 3, tempRankUp1[i] );
                cFILE.getflag( "dwRankUp2", 1+i, 3, tempRankUp2[i] );
                cFILE.getflag( "dwSPReduce", 1+i, 3, tempSPReduce[i]);
                cFILE.getflag( "dwAddAbility", 1+i, 3, tempAddAbility[i] );
                cFILE.getflag( "dwRandomAbility", 1+i, 3, tempRandomAbility[i] );
                cFILE.getflag( "dwElementalAbility", 1+i, 3, tempElementalAbility[i] );
                cFILE.getflag( "dwTrade", 1+i, 3, tempTrade[i] );
                cFILE.getflag( "dwLimitTime", 1+i, 3, tempLimitTime[i] );
				cFILE.getflag( "dwAddDisguise", 1 + i, 3, tempAddDisguise[ i ] );
				cFILE.getflag( "dwMoney", 1 + i, 3, tempMoney[ i ] );
				cFILE.getflag( "dwSetItem", 1 + i, 3, tempSetItem[ i ] );
            }
	...
	BOOL LOAD_WEAPON_LINKSKILL(char* szFileName) 
	{
		std::string strPath;
		strPath = GLOGIC::GetPath();
		strPath += szFileName;
		...
		if( !cFILE.open( strPath, true, GLOGIC::bGLOGIC_PACKFILE ) )
		{
			CDebugSet::ToLogFile ( "ERROR : GLCONST_CHAR::LOAD_WEAPON_LINKSKILL()" );
			return FALSE;
		}

		INT nKeySize(0);

		cFILE.getflag( "WEAPON_LINKSKILL_INFO", "KeySize", 0, 1, nKeySize );

		DWORD dwNUM = cFILE.GetKeySize( "WEAPON_LINKSKILL_LIST", "LINKSKILL_INFO" );
		for ( DWORD i=0; i<dwNUM; ++i )
		{
			GLWEAPON_LINKSKILL	glSkill;

			cFILE.getflag(i, "WEAPON_LINKSKILL_LIST", "LINKSKILL_INFO", 0, nKeySize, glSkill.sNativeID.wMainID);
			cFILE.getflag( i, "WEAPON_LINKSKILL_LIST", "LINKSKILL_INFO", 1, nKeySize, glSkill.sNativeID.wSubID );

			cFILE.getflag( i, "WEAPON_LINKSKILL_LIST", "LINKSKILL_INFO", 2, nKeySize, glSkill.fPossibility );
			cFILE.getflag( i, "WEAPON_LINKSKILL_LIST", "LINKSKILL_INFO", 3, nKeySize, glSkill.fRate );

			vecWeaponLinkSkill.push_back(glSkill);
		}
