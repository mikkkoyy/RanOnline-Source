...
HRESULT GLCharAG::FrameMove ( float fTime, float fElapsedTime )
{
	...
	if ( m_bDeathTimer )
	{
		...
		//revive fix bypass for war maps
		if ( pLandMan->m_bPVPTyrannyMap )
		{
			if ( m_fDeathTimer >= 14.0f )
			{
				m_bDeathTimer = FALSE;
				m_fDeathTimer = 0.0f;
			}
		}
		else if ( pLandMan->m_bPVPSchoolWarsMap || pLandMan->m_bPVPCaptureTheFlagMap )
		{
			if ( m_fDeathTimer >= 9.0f )
			{
				m_bDeathTimer = FALSE;
				m_fDeathTimer = 0.0f;
			}
	...
	if ( m_fFRIEND_TIMER > 2.f )
	{
		...
		for ( ; iter!=m_mapFriend.end(); ++iter )
		{
			...
			bool bONLINE(false);
			int nCHANNEL(0);
			WORD wLevel = 0;/*PartySearch - Added By Bubu*/
			...
			if( ( bONLINE!=cFRIEND.bONLINE ) || ( nCHANNEL!=cFRIEND.nCHANNEL) )
			{
				...
				cFRIEND.m_wLevel = wLevel;/*PartySearch - Added By Bubu*/

				//	note : 메시지 발생.				
				NetMsg.nChaFlag = cFRIEND.nCharFlag;
				NetMsg.bONLINE = cFRIEND.bONLINE;
				NetMsg.nCHANNEL = cFRIEND.nCHANNEL;
				NetMsg.m_wLevel = cFRIEND.m_wLevel;/*PartySearch - Added By Bubu*/
	...
	if ( m_bDeathTimer )
	{
		...
		if( pLandMan )
		{
			if( pLandMan->m_bPVPTyrannyMap )		fDeathTime = TYRANNY_REVIVE_TIME;
			if( pLandMan->m_bPVPSchoolWarsMap )		fDeathTime = SCHOOLWARS_REVIVE_TIME;
			if( pLandMan->m_bPVPCaptureTheFlagMap )	fDeathTime = CAPTURE_THE_FLAG_REVIVE_TIME;
		}
...
void GLCharAG::AddFriend ( char *szFriend )
{
	...
	sFRIEND.m_wLevel = pChar->m_wLevel;/*PartySearch - Added By Bubu*/
...
//	pFRIEND->nCharFlag = dwFlag;
	...
	NetMsg.m_wLevel = pCHAR->m_wLevel;/*PartySearch - Added By Bubu*/
