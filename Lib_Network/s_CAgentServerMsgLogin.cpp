...
void CAgentServer::ThaiMsgLogInBack( NET_LOGIN_FEEDBACK_DATA2* nlfd2 )
{
	...
	if (nlfd2->nResult == EM_LOGIN_FB_SUB_OK)
	{
			 ...
			 (strcmp(m_pClientManager->GetUserID(dwClient),   nlfd2->szUserid) == 0))
		{
			...
			m_pClientManager->SetGainGameTime ( dwClient, nlfd2->bGameTime );
...
void CAgentServer::ChinaMsgLoginBack( NET_LOGIN_FEEDBACK_DATA2* nlfd2 )
{
	...
	if (nlfd2->nResult == EM_LOGIN_FB_SUB_OK)
	{
			...
			(strcmp( m_pClientManager->GetUserID( dwClient ),   nlfd2->szUserid ) == 0))
		{
			...
			/*dmk14 ingame web*/
			m_pClientManager->SetPremiumPoints( dwClient, nlfd2->dwPremiumPoints );
			m_pClientManager->SetCombatPoints( dwClient, nlfd2->dwCombatPoints );

			m_pClientManager->SetAccType( dwClient, nlfd2->emUserType );

			nlfd.dwGameTime = nlfd2->dwGameTime;
			nlfd.dwPremiumPoint = nlfd2->dwPremiumPoints;
			nlfd.dwCombatPoint = nlfd2->dwCombatPoints;
...
{		
	...
	*/
	...
	StringCchCopy (szUserid, USR_ID_LENGTH+1, pNml->szUserid);
	StringCchCopy (szPassword, USR_PASS_LENGTH+wPassLength, pNml->szPassword);
	StringCchCopy (szRandomPassword, USR_RAND_PASS_LENGTH+1, pNml->szRandomPassword);
