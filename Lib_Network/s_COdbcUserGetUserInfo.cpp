	...
	NET_LOGIN_FEEDBACK_DATA2* pMsg )
{
    ...
    SQLINTEGER nUserNum       = 0, cbUserNum       = SQL_NTS;
    SQLINTEGER nUserType      = 0, cbUserType      = SQL_NTS;
	SQLINTEGER nUserType2      = 0, cbUserType2      = SQL_NTS;
    SQLINTEGER nChaRemain     = 0, cbChaRemain     = SQL_NTS;
    SQLINTEGER nChaTestRemain = 0, cbChaTestRemain = SQL_NTS;
 ...
 //   std::strstream strTemp;
	...
	/*disable old query get user info
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 256, "SELECT UserNum, UserType, PremiumDate, ChaRemain, "
	...
	else
	{
		...
		while(true)
		{
			...
			{	
				::SQLGetData(pConn->hStmt, 1, SQL_C_LONG,           &nUserNum,       0, &cbUserNum);
                ::SQLGetData(pConn->hStmt, 2, SQL_C_LONG,           &nUserType,      0, &cbUserType);
				::SQLGetData(pConn->hStmt, 3, SQL_C_LONG,           &nUserType2,      0, &cbUserType2);
                ::SQLGetData(pConn->hStmt, 4, SQL_C_TYPE_TIMESTAMP, &sPremiumDate,   0, &cbPremiumDate);
                ::SQLGetData(pConn->hStmt, 5, SQL_C_LONG,           &nChaRemain,     0, &cbChaRemain);
                ::SQLGetData(pConn->hStmt, 6, SQL_C_LONG,           &nChaTestRemain, 0, &cbChaTestRemain);
				::SQLGetData(pConn->hStmt, 7, SQL_C_TYPE_TIMESTAMP, &sChatBlockDate, 0, &cbChatBlockDate);

				::SQLGetData(pConn->hStmt, 8, SQL_C_LONG,           &nChinaGameTime, 0, &cbChinaGameTime);
				::SQLGetData(pConn->hStmt, 9, SQL_C_LONG,           &nChinaOfflineTime, 0, &cbChinaOfflineTime);
				::SQLGetData(pConn->hStmt, 10, SQL_C_TYPE_TIMESTAMP, &sLastLoginTime, 0, &cbLastLoginTime);
				::SQLGetData(pConn->hStmt, 11, SQL_C_LONG,           &nChinaUserAge, 0, &cbChinaUserAge);


                pMsg->nUserNum       = nUserNum;
                pMsg->nUserType      = nUserType;
				pMsg->emUserType	 = EMACCTYPE ( nUserType2 );
				pMsg->uChaRemain     = (USHORT) nChaRemain;
                pMsg->uChaTestRemain = (USHORT) nChaTestRemain;
		...
		while(true)
		{
			...
			{	
				...
				::SQLGetData(pConn->hStmt, 7, SQL_C_TYPE_TIMESTAMP, &sChatBlockDate, 0, &cbChatBlockDate);
				/*dmk14 ingame web*/
				::SQLGetData(pConn->hStmt, 8, SQL_C_LONG,           &nPlayTime,		 0, &cbPlayTime);
				::SQLGetData(pConn->hStmt, 9, SQL_C_LONG,           &nUserPoint,     0, &cbUserPoint);
				::SQLGetData(pConn->hStmt, 10, SQL_C_LONG,           &nUserPoint2,     0, &cbUserPoint2);
				::SQLGetData(pConn->hStmt, 11, SQL_C_CHAR, szEmail   ,USR_INFOMAIL_LENGTH+1,&cbEmail); 
				...
				/*dmk14 ingame web*/
				pMsg->dwPremiumPoints	= nUserPoint;
				pMsg->dwCombatPoints	= nUserPoint2;
	...
	_snprintf( szTemp, 512, "SELECT ChaLevel FROM ChaInfo Where UserNum='%d' and ChaDeleted = 0", nUserNum );
    ...
    while(true)
	{
		...
		if (sReturn == SQL_SUCCESS || sReturn == SQL_SUCCESS_WITH_INFO)
		{   
			::SQLGetData(pConn->hStmt, 1, SQL_C_LONG, &nChaLevel, 0, &cbChaLevel );
			if( nChaLevel >= GLCONST_CHAR::wLEVEL_REQ_GAMETIME)	pMsg->bGameTime = true;
		}
		else
		{
			break;
