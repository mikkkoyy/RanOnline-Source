...
{	
	...
	case NET_MSG_GCTRL_PARTY_SEARCH_LURE:
	case NET_MSG_GCTRL_PARTY_SEARCH_LURE_TAR_ANS:
	...
	/*PartySearch - Added By Bubu*/
	case NET_MSG_GCTRL_GET_PARTYLIST:
...
void CAgentServer::OtherMsgProcess(MSG_LIST* pMsg)
{
	...
	/*PartySearch - Added By Bubu*/
	case NET_MSG_GCTRL_REQ_PARTY_SEARCH_UPDATE:
	...
	default :
		CConsoleMessage::GetInstance()->Write( _T("Agt Msg Leak Type ( %d ) Client ID ( %d )"), nmg->nType, pMsg->dwClient );		
		break;
	};
}
...
void CAgentServer::MsgGameJoin(MSG_LIST* pMsg)
{
	...
	/*dmk14 ingame web*/
	DWORD dwPremiumPoints = m_pClientManager->GetPremiumPoints(dwClient);
	DWORD dwCombatPoints = m_pClientManager->GetCombatPoints(dwClient);

	EMACCTYPE emUserType = m_pClientManager->GetAccType(dwClient);
	...
	pDBAction->Init ( dwClient, dwUserID, dwUserLvl, tPREMIUM, tCHATBLOCK, (DWORD)m_nServerGroup, dwChaNum, dwPremiumPoints, dwCombatPoints, emUserType );
