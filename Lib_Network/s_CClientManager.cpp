...
void CClientManager::SetGainGameTime( DWORD dwClient, bool bGain )
{
	m_pClient[dwClient].SetGainGameTime( bGain );
}

bool CClientManager::GetGainGameTime( DWORD dwClient )
{
	return m_pClient[dwClient].GetGainGameTime();
}
/*
...
void CClientManager::SetAccType( DWORD dwClient, EMACCTYPE emAccType )
{
	m_pClient[dwClient].SetAccType( emAccType );
}

EMACCTYPE CClientManager::GetAccType( DWORD dwClient )
{
	return m_pClient[dwClient].GetAccType();
}
...
//iplimit
void CClientManager::IPSetClientMax( WORD _wMAX, bool bEnableBlock )
{
	m_wClientIPMax = _wMAX;
	bBlockCMD = bEnableBlock;	/*dmk14 blocker*/
	if ( m_wClientIPMax == 0 ){
