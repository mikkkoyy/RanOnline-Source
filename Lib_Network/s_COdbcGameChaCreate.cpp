...
int COdbcManager::CreateNewCharacter(SCHARDATA2* pCharData2)
{
	...
	_snprintf( szTemp, 2048, "INSERT INTO ChaInfo(UserNum, SGNum, ChaName, ChaTribe, ChaClass, "
		...
		pCharData2->m_bEnableStorageLock,	/*storage lock, Juver, 2018/01/13 */
		pCharData2->m_bStorageLockStatus,	/*storage lock, Juver, 2018/01/13 */

		pCharData2->m_bEnableInventoryLock,	/*inventory lock, Juver, 2018/01/13 */
		pCharData2->m_bInventoryLockStatus	/*inventory lock, Juver, 2018/01/13 */
...
//		Print(GetErrorString(pConn2->hStmt));
	...
	*/
	...
	{
		TCHAR szTemp[256] = {0};
		_snprintf( szTemp, 256, "UPDATE ChaInfo SET ChaInfo.ChaItemFood=? WHERE (ChaNum=%d)", nChaNewNum );
		m_pGameDB->WriteImage( szTemp, nChaNewNum, (BYTE *) pBuffer, dwSize );
		pBuffer = NULL;
	}	
