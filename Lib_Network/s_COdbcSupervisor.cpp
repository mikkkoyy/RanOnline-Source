...
// PHASE2 : parameterized stored procedure call. Binds the caller supplied
// inputs as parameters 1..nParamCount and the trailing ? of szSP as the
// SQL_PARAM_OUTPUT result, then executes. Mirrors ExecuteSpInt( const TCHAR* )
// above, which supports the output parameter only.
int COdbcSupervisor::ExecuteSpInt( const TCHAR * szSP, const SP_PARAM * pParams, int nParamCount )
{
	if ( pParams == NULL || nParamCount < 0 || nParamCount > MAX_SP_INPUT_PARAM )
		return DB_ERROR;
	...
	// cbValue / cbValueLen must stay alive until SQLExecute has run.
	SQLLEN	cbValue[MAX_SP_INPUT_PARAM];
	SQLLEN	cbValueLen[MAX_SP_INPUT_PARAM];
	...
	for ( int i = 0; i < nParamCount; i++ )
	{
		...
		if ( pParams[i].eType == SP_PARAM::TYPE_STRING )
		{
			const TCHAR* szValue = ( pParams[i].szValue != NULL ) ? pParams[i].szValue : _T("");
			cbValueLen[i] = (SQLLEN)( _tcslen( szValue ) );

			// (SQLPOINTER) also strips the const qualifier; harmless because an
			// SQL_PARAM_INPUT buffer is only read by the driver.
			sReturn = ::SQLBindParameter(
						pConn->hStmt,
						(SQLUSMALLINT)( i + 1 ),
						SQL_PARAM_INPUT,
