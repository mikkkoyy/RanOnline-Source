...
CDamageDisplay::CDamageDisplay () :
	...
	m_pIMMUNE_RENDER ( NULL ),
	m_pABSORB_RENDER ( NULL ),
	m_pIMMUNE( NULL ),
	m_pABSORB( NULL )
{
...
void CDamageDisplay::CreateSubControl ()
{
	...
	{	//	문자
		...
		m_pIMMUNE  = CreateControl ( "DAMAGE_DISPLAY_IMMUNE" );
		m_pABSORB  = CreateControl ( "DAMAGE_DISPLAY_ABSORB" );
	}

	{
		...
		m_pIMMUNE_RENDER  = CreateControl ( "TRASHCONTROL" );
		m_pABSORB_RENDER  = CreateControl ( "TRASHCONTROL" );
	}
...
HRESULT CDamageDisplay::ResetRenderData ()
{
	...
	m_pIMMUNE_RENDER->SetVisibleSingle ( FALSE );	
	m_pABSORB_RENDER->SetVisibleSingle ( FALSE );	
...
//			m_fScale = 1.2f;
	...
	else	//	MISS
	{
		...
		if ( dwDamageFlag & DAMAGE_TYPE_DAMAGE_ABSORBED )
		{
			m_fScale = 0.5f;

			CloneControl ( m_pABSORB_RENDER, m_pABSORB );
			m_pABSORB_RENDER->SetVisibleSingle ( TRUE );	//	critical
			{	//	Scale
				UIRECT rcOriginPos = m_pABSORB_RENDER->GetLocalPos ();
				float fSCALE_WIDTH = floor ( rcOriginPos.sizeX * m_fScale );
				float fSCALE_HEIGHT = floor ( m_pDUMMY->GetLocalPos ().sizeY * m_fScale );
				UIRECT rcScalePos = UIRECT ( vStartPos.x, fTOP, fSCALE_WIDTH, fSCALE_HEIGHT );
				m_pABSORB_RENDER->SetLocalPos ( rcScalePos );

				vStartPos.x = m_pABSORB_RENDER->GetLocalPos ().sizeX;
				vStartPos.y = m_pABSORB_RENDER->GetLocalPos ().sizeY;
			}
		}
		else if ( dwDamageFlag & DAMAGE_TYPE_IMMUNE )
		{
			m_fScale = 0.5f;
			CloneControl ( m_pIMMUNE_RENDER, m_pIMMUNE );
			m_pIMMUNE_RENDER->SetVisibleSingle ( TRUE );	//	IMMUNE
			{	//	Scale
				UIRECT rcOriginPos = m_pIMMUNE_RENDER->GetLocalPos ();
				float fSCALE_WIDTH = floor ( rcOriginPos.sizeX * m_fScale );
				float fSCALE_HEIGHT = floor ( m_pDUMMY->GetLocalPos ().sizeY * m_fScale );
				UIRECT rcScalePos = UIRECT ( vStartPos.x, fTOP, fSCALE_WIDTH, fSCALE_HEIGHT );
				m_pIMMUNE_RENDER->SetLocalPos ( rcScalePos );

				vStartPos.x = m_pIMMUNE_RENDER->GetLocalPos ().sizeX;
				vStartPos.y = m_pIMMUNE_RENDER->GetLocalPos ().sizeY;
			}
		}
		else
		{
			m_fScale = 0.5f;
			CloneControl ( m_pMISS_RENDER, m_pMISS );
			m_pMISS_RENDER->SetVisibleSingle ( TRUE );	//	MISS
			{	//	Scale
				UIRECT rcOriginPos = m_pMISS_RENDER->GetLocalPos ();
				float fSCALE_WIDTH = floor ( rcOriginPos.sizeX * m_fScale );
				float fSCALE_HEIGHT = floor ( m_pDUMMY->GetLocalPos ().sizeY * m_fScale );
...
BOOL CDamageDisplay::SetFadeOut ()
{
	...
	if ( m_pIMMUNE_RENDER->IsVisible () )		m_pIMMUNE_RENDER->SetFadeOut ();
	if ( m_pABSORB_RENDER->IsVisible () )		m_pABSORB_RENDER->SetFadeOut ();
