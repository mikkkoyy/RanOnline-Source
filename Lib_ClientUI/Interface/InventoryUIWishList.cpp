...
const int CInventoryUIWishList::nSTARTLINE = 0;
const int CInventoryUIWishList::nOUTOFRANGE = -1;

CInventoryUIWishList::CInventoryUIWishList ()
	: m_nCurPos ( -1 ),
	m_pHelp(NULL)
{
...
CUIControl*	CInventoryUIWishList::CreateControl ( const char* szControl )
{
...
void CInventoryUIWishList::CreateSubControl ()
{
	...
	for ( int i = 0; i < WISHLIST_INVEN_Y; i++ )
	{
...
void CInventoryUIWishList::SetVisibleSingle ( BOOL bVisible )
{
...
void CInventoryUIWishList::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	SetItemIndex ( nOUTOFRANGE, nOUTOFRANGE );
	CUIGroup::Update ( x, y, LB, MB, RB, nScroll, fElapsedTime, bFirstControl );
}

void CInventoryUIWishList::ResetAllItemSlotRender ( int index, int nTotal )
{
	if ( nTotal < 0 ) return ;
...
void CInventoryUIWishList::SetItemSlotRender ( int nStartIndex, int nTotal )
{
	if ( nTotal < 0 ) return ;
...
CItemSlot* CInventoryUIWishList::CreateItemSlot ( CString strKeyword, UIGUID ControlID )
{
	CItemSlot* pItemSlot = new CItemSlot;
	pItemSlot->CreateSub ( this, strKeyword.GetString (), UI_FLAG_DEFAULT, ControlID );
	pItemSlot->CreateSubControl ( WISHLIST_INVEN_X, TRUE );
	RegisterControl ( pItemSlot );

	return pItemSlot;
}

void CInventoryUIWishList::LoadItem ( SINVENITEM& ref_InvenItem )
{	
...
void CInventoryUIWishList::LoadItemPage ( GLInventory &ItemData )
{
	GLInventory::CELL_MAP *pItemList = ItemData.GetItemList();
	
	if ( pItemList->size () > (WISHLIST_INVEN_Y * WISHLIST_INVEN_X) )
	{
...
void CInventoryUIWishList::UnLoadItemPage ()
{
	for ( int y = 0; y < WISHLIST_INVEN_Y; y++ )
	{
		for ( int x = 0; x < WISHLIST_INVEN_X; x++ )
		{
			UnLoadItem ( x, y );
		}
	}
...
SINVENITEM& CInventoryUIWishList::GetItem ( int nPosX, int nPosY )
{
	return m_pItemSlotArray[nPosY]->GetItemImage ( nPosX );
}

void CInventoryUIWishList::UnLoadItem ( int nPosX, int nPosY )
{
	m_pItemSlotArray[nPosY]->ResetItemImage ( nPosX );
}

void CInventoryUIWishList::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	...
	case HELP_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )	
			{
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO( ID2GAMEINTEXT( "INVENTORY_HELP_WISH"), NS_UITEXTCOLOR::WHITE );
			}
		}break;
	};
}

void CInventoryUIWishList::SetItemIndex ( int nPosX, int nPosY )
{
	m_nPosX = nPosX;
	m_nPosY = nPosY;
}

void CInventoryUIWishList::GetItemIndex ( int* pnPosX, int* pnPosY )
{
	*pnPosX = m_nPosX;
	*pnPosY = m_nPosY;
}

void CInventoryUIWishList::OpenPage ( int nPage )
{
	for ( int i = 0 ; i < nMAX_PAGES; ++i )	
	{
		m_pPages[i]->SetFlip ( false );
