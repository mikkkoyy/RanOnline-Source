...
CBasicTextButton*  CInventoryWindow::CreateTextButtonClick ( const char* szButton, UIGUID ControlID, const char* szText )
{
...
//				if ( CUIMan::GetFocusControl () == this )
				...
				/* CTRL + Left Click - Sell item, Nell, 2021/02/12 */
				if ( bMarketOpen )
				{
...
//							int nPosY = nOUTOFRANGE;
				...
				/* CTRL + Left Click - Sell item END, Nell, 2021/02/12 */
	...
	case INVENTORY_TRASH_BUTTON:
		{
			...
			if ( CHECK_MOUSE_IN ( dwMsg ) )	
			{
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO( ID2GAMEWORD( "INVENTORY_BUTTON_STATIC" ,2 ) , NS_UITEXTCOLOR::WHITE );
			}
	...
	case INVENTORY_SORT_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )	
			{
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO( ID2GAMEWORD( "INVENTORY_BUTTON_STATIC" ,3 ) , NS_UITEXTCOLOR::WHITE );
			}
