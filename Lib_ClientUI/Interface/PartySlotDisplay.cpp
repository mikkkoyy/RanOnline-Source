...
void CPartySlotDisplay::CreateSubControl ()
{
...

	{
		...
		CString strClassKeyword[GLCI_NUM_8CLASS] = 
		{
			...
			"PARTYDISPLAY_SLOT_CLASS_ASSASIN_M",
			"PARTYDISPLAY_SLOT_CLASS_ASSASIN_W",
	...
	
	{
			...
			pProgressBar12->CreateOverImage ( "PARTYDISPLAY_PROGRESSBAR_SLOT_CLASS_ASSASIN_M" );
			...
			pProgressBar13->CreateOverImage ( "PARTYDISPLAY_PROGRESSBAR_SLOT_CLASS_ASSASIN_W" );
...
void CPartySlotDisplay::SetPlayerHealth ( int fHPNow, int fHPMax, CString strName )
{
	CString strHP; strHP.Format ( "%d/%d",fHPNow,fHPMax ); 
	if ( fHPNow == 0 ) 
	{
		m_pPlayerName->SetOneLineText( strName, NS_UITEXTCOLOR::RED );
