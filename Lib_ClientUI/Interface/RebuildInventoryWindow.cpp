...
VOID CRebuildInventoryWindow::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
	...
	/*dmk14 reform*/
	if( ref_CharData.m_sRebuildItem.VALID() || ref_CharData.m_sRebuildItemSealed != NATIVEID_NULL() )
	{
		if( ref_CharData.m_sRebuildItem.VALID() )		m_pPage->UpdateFlipItem( ref_CharData.m_sRebuildItem );
		
		if( ref_CharData.m_sRebuildItemSealed != NATIVEID_NULL() )	
		{
			SINVENITEM* pResistItem = ref_CharData.m_cInventory.FindItem( ITEM_SEALED_CARD, ref_CharData.m_sRebuildItemSealed );
			if( pResistItem ) m_pPage->UpdateFlipItem( SINVEN_POS(pResistItem->wPosX, pResistItem->wPosY) );
		}
	}
	else
