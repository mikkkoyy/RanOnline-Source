...
const INT CInventoryUI_RebuildInven::nSTARTLINE = 0;
const INT CInventoryUI_RebuildInven::nOUTOFRANGE = -1;

CInventoryUI_RebuildInven::CInventoryUI_RebuildInven()
...
VOID CInventoryUI_RebuildInven::CreateSubControl()
{
...
CItemSlotEx_Renew* CInventoryUI_RebuildInven::CreateItemSlot( CString strKeyword, UIGUID ControlID )
{
...
CUIControl*	CInventoryUI_RebuildInven::CreateControl( const TCHAR* szControl )
{
...
VOID CInventoryUI_RebuildInven::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
...
VOID CInventoryUI_RebuildInven::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
	...
	case HELP_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )	
			{
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO( ID2GAMEINTEXT( "INVENTORY_REBUILD_HELP_INVEN" ) , NS_UITEXTCOLOR::WHITE );
			}
		}break;
	};
}

void CInventoryUI_RebuildInven::SetVisibleSingle ( BOOL bVisible )
{
...
VOID CInventoryUI_RebuildInven::ResetAllItemSlotRender( INT nStartIndex, INT nTotal )
{
	if( nTotal < 0 )
		return;
...
VOID CInventoryUI_RebuildInven::SetItemSlotRender( INT nStartIndex, INT nTotal )
{
	if( nTotal < 0 )
		return;
...
VOID CInventoryUI_RebuildInven::SetItemIndex( INT nPosX, INT nPosY )
{
	m_nPosX = nPosX;
	m_nPosY = nPosY;
}

VOID CInventoryUI_RebuildInven::GetItemIndex( INT* pnPosX, INT* pnPosY )
{
	*pnPosX = m_nPosX;
	*pnPosY = m_nPosY;
}

VOID CInventoryUI_RebuildInven::LoadItemPage( GLInventory &ItemData )
{
...
VOID CInventoryUI_RebuildInven::LoadItem( SINVENITEM& ref_InvenItem )
{	
...
VOID CInventoryUI_RebuildInven::UnLoadItemPage()
{
...
VOID CInventoryUI_RebuildInven::UnLoadItem( INT nPosX, INT nPosY )
{
	m_pItemSlotArray[ nPosY ]->ResetItemImage( nPosX );
}

VOID CInventoryUI_RebuildInven::ResetAllFlipItem()
{
	for( WORD y=0; y<EM_INVENSIZE_Y; ++y )
		for( WORD x=0; x<EM_INVENSIZE_X; ++x )
...
VOID CInventoryUI_RebuildInven::UpdateFlipItem( GLInventory& ref_Inventory )
{
	ResetAllFlipItem();
...
VOID CInventoryUI_RebuildInven::UpdateFlipItem( SINVEN_POS& ref_RebuildItem )
{
	//ResetAllFlipItem();
...
VOID CInventoryUI_RebuildInven::UpdateBLOCK()
{
...
SINVENITEM& CInventoryUI_RebuildInven::GetItem( INT nPosX, INT nPosY )
{
	return m_pItemSlotArray[ nPosY ]->GetItemImage( nPosX );
}

VOID CInventoryUI_RebuildInven::SetOneViewSlot( const INT& nMAX_ONE_VIEW )
{
	m_nONE_VIEW_SLOT = nMAX_ONE_VIEW;
	m_nCurPos = -1;
}

void CInventoryUI_RebuildInven::OpenPage ( int nPage )
{
	for ( int i = 0 ; i < nMAX_PAGES; ++i )	
	{
		m_pPages[i]->SetFlip ( false );
