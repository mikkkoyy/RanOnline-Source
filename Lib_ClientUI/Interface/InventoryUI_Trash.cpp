...
// CInventoryUI_Trash
//--------------------------------------------------------------------
CInventoryUI_Trash::CInventoryUI_Trash()	// »ﬁ¡ˆ≈Î
	: m_pPage( NULL )
	, m_nONE_VIEW_SLOT( 4 )
	, m_pInvenButton( NULL )
...
CInventoryUI_Trash::~CInventoryUI_Trash()
{
}

VOID CInventoryUI_Trash::CreateSubControl()
{
	...
	m_pPage = new CInventoryUI_TrashInven;
	m_pPage->CreateSub( this, "INVENTORY_WINDOW_ITEM_REGION", UI_FLAG_YSIZE, GARBAGEINVENTORY_PAGE );
	m_pPage->CreateSubControl();
	RegisterControl( m_pPage );

	m_pInvenButton = CreateTextButton23 ( "INVENTORY_WINDOW_INVEN_TAP", GARBAGEINVENTORY_INVEN_BUTTON, (char*)ID2GAMEWORD ( "INVENTORY_TAP_NAME", 0 ) );
	m_pInvenButton->SetFlip( TRUE );

	CBasicButton* pMoneyButton = NULL;
	pMoneyButton = CreateFlipButton ( "INVENTORY_MONEY_BUTTON_RENEW", "INVENTORY_MONEY_BUTTON_RENEW_F", GARBAGEINVENTORY_MONEY_BUTTON, CBasicButton::CLICK_FLIP );
	...
	pPointButton = CreateFlipButton ( "INVENTORY_POINT_BUTTON_RENEW", "INVENTORY_POINT_BUTTON_RENEW_F", GARBAGEINVENTORY_POINT_BUTTON, CBasicButton::CLICK_FLIP );
...
CBasicTextButton*  CInventoryUI_Trash::CreateTextButton23 ( const char* szButton, UIGUID ControlID, const char* szText )
{
...
VOID CInventoryUI_Trash::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
...
VOID CInventoryUI_Trash::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
	switch( ControlID )
	{
...
HRESULT CInventoryUI_Trash::RestoreDeviceObjects( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
VOID CInventoryUI_Trash::SetOneViewSlot( const INT& nONE_VIEW_SLOT )
{
