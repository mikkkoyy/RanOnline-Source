...
const INT CInventoryUI_ItemMixInven::nSTARTLINE = 0;	
const INT CInventoryUI_ItemMixInven::nOUTOFRANGE = -1;

CInventoryUI_ItemMixInven::CInventoryUI_ItemMixInven()
...
VOID CInventoryUI_ItemMixInven::CreateSubControl()
{
	...
	/*
	m_pPages = new CBasicButton;
	m_pPages->CreateSub ( this, "INVENTORY_WINDOW_RADIO_BUTTON_1", UI_FLAG_DEFAULT, PAGE_BUTTON0 );
	m_pPages->CreateFlip ( "INVENTORY_WINDOW_RADIO_BUTTON_F", CBasicButton::RADIO_FLIP );
	m_pPages->SetFlip( true );
	m_pPages->SetVisibleSingle ( TRUE );
	RegisterControl ( m_pPages );

	m_pPagesText = new CBasicTextBox;
	m_pPagesText->CreateSub ( this, "INVENTORY_WINDOW_RADIO_BUTTON_TEXT_1", UI_FLAG_DEFAULT );
	m_pPagesText->SetFont ( pFont8 );
	m_pPagesText->SetTextAlign ( TEXT_ALIGN_LEFT );	
	m_pPagesText->AddText ( "1", NS_UITEXTCOLOR::WHITE );
	RegisterControl ( m_pPagesText );
	*/
...
CItemSlotEx_Renew* CInventoryUI_ItemMixInven::CreateItemSlot( CString strKeyword, UIGUID ControlID )
{
...
CUIControl*	CInventoryUI_ItemMixInven::CreateControl( const TCHAR* szControl )
{
...
VOID CInventoryUI_ItemMixInven::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
...
void CInventoryUI_ItemMixInven::SetVisibleSingle ( BOOL bVisible )
{
...
VOID CInventoryUI_ItemMixInven::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
	...
	case HELP_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )	
			{
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO( ID2GAMEINTEXT( "INVENTORY_ITEMMIX_HELP_INVEN" ) , NS_UITEXTCOLOR::WHITE );
			}
		}break;
	};
}

VOID CInventoryUI_ItemMixInven::ResetAllItemSlotRender( INT nStartIndex, INT nTotal )
{
	if( nTotal < 0 )
		return;
...
VOID CInventoryUI_ItemMixInven::SetItemSlotRender( INT nStartIndex, INT nTotal )
{
	if( nTotal < 0 )
		return;
...
VOID CInventoryUI_ItemMixInven::SetItemIndex( INT nPosX, INT nPosY )
{
	m_nPosX = nPosX;
	m_nPosY = nPosY;
}

VOID CInventoryUI_ItemMixInven::GetItemIndex( INT* pnPosX, INT* pnPosY )
{
	*pnPosX = m_nPosX;
	*pnPosY = m_nPosY;
}

VOID CInventoryUI_ItemMixInven::LoadItemPage( GLInventory &ItemData )
{
...
VOID CInventoryUI_ItemMixInven::LoadItem( SINVENITEM& ref_InvenItem )
{	
...
VOID CInventoryUI_ItemMixInven::UnLoadItemPage()
{
...
VOID CInventoryUI_ItemMixInven::UnLoadItem( INT nPosX, INT nPosY )
{
	m_pItemSlotArray[ nPosY ]->ResetItemImage( nPosX );
}

VOID CInventoryUI_ItemMixInven::ResetAllFlipItem()
{
	for( WORD y=0; y<EM_INVENSIZE_Y; ++y )
		for( WORD x=0; x<EM_INVENSIZE_X; ++x )
...
VOID CInventoryUI_ItemMixInven::UpdateFlipItem( GLInventory& ref_Inventory )
{
	ResetAllFlipItem();
...
VOID CInventoryUI_ItemMixInven::UpdateFlipItem( SINVEN_POS& ref_GarbageItem )
{
...
VOID CInventoryUI_ItemMixInven::UpdateBLOCK()
{
...
SINVENITEM& CInventoryUI_ItemMixInven::GetItem( INT nPosX, INT nPosY )
{
	return m_pItemSlotArray[ nPosY ]->GetItemImage( nPosX );
}

VOID CInventoryUI_ItemMixInven::SetOneViewSlot( const INT& nMAX_ONE_VIEW )
{
	m_nONE_VIEW_SLOT = nMAX_ONE_VIEW;
	m_nCurPos = -1;
}

void CInventoryUI_ItemMixInven::OpenPage ( int nPage )
{
	for ( int i = 0 ; i < nMAX_PAGES; ++i )	
	{
		m_pPages[i]->SetFlip ( false );
