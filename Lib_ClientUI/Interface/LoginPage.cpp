...
//#endif
	...
	pForgotButton->CreateSub ( this, "BASIC_TEXT_BUTTON18", UI_FLAG_XSIZE, LOGIN_FORGOT );
	pForgotButton->CreateBaseButton ( "LOGIN_PAGE_FORGOT", nBUTTONSIZE, CBasicButton::CLICK_FLIP, (char*)ID2GAMEWORD ( "LOGIN_PAGE_OKCANCEL", 3 ) );
	pForgotButton->SetVisibleSingle( RANPARAM::bFeatureRegister );
	...
	case LOGIN_FORGOT:
		{
			if ( CHECK_KEYFOCUSED ( dwMsg ) || CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
			{
				COuterInterface::GetInstance().ToForgotPassPage ( GetWndID () );
			}
		}break;
