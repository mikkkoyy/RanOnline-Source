...
void CTradeInventoryWindow::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	m_bREBUILD_MODE = (CInnerInterface::GetInstance().IsVisibleGroup(ITEM_REBUILD_WINDOW))?true:false;
...
//				}
				...
				if ( dwMsg & UIMSG_LB_UP )
				{   
					if ( m_bTRADE_MODE || m_bREBUILD_MODE )	//	거래모드
					{
	...
	case ET_CONTROL_BUTTON:
		{
			...
			{				
				...
				else if ( m_bREBUILD_MODE ) CInnerInterface::GetInstance().CloseItemRebuildWindow();
				else
				{
					bool bOPENER;
					DWORD dwGaeaID;
