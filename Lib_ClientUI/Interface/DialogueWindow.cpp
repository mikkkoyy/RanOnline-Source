...
void CDialogueWindow::LoadBasicTalk ()
{
	...
	if ( RANPARAM::strLangPlacer != "en" )
	{
		//xml translation
		CString strTemp;				
		std::transform ( m_strTalkFile.begin(), m_strTalkFile.end(), m_strTalkFile.begin(), tolower );
		strTemp.Format ( "%s_%d_%d", m_strTalkFile.c_str(), m_pDialogue->GetDlgNID(), m_dwIndex );
		m_pQuestionBox->SetText ( ID2GAMEWORDNPC( strTemp ), NS_UITEXTCOLOR::BURLYWOOD );
	}
	else
	{
	...
	if ( pNpcTalkControl )
	{
		...
		for ( ; pos != pos_end; ++pos )
		{
			...
			if ( RANPARAM::strLangPlacer != "en" )
			{
				...
				CString strTemp;				
				std::transform ( m_strTalkFile.begin(), m_strTalkFile.end(), m_strTalkFile.begin(), tolower );
				strTemp.Format ( "%s_%d_%d_%d", m_strTalkFile.c_str(), m_pDialogue->GetDlgNID(), m_dwIndex,pNpcTalk->m_dwNID );
				int nIndex = m_pAnswerBox->AddText ( ID2GAMEWORDNPC( strTemp ), dwColor );
