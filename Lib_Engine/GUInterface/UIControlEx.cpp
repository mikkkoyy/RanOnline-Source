...
void CUIControl::ResetControl( const UIRECT& rcParentGlobalPos, const char* szControlKeyword )
{
    ...
    pControl->Create ( 1, szControlKeyword );
    
    if ( !pControl->GetTextureName().IsEmpty() )
    {
        CUIControl::SetTextureName ( pControl->GetTextureName() );
        CUIControl::SetTexturePos  ( pControl->GetTexturePos() );
    }
