...
CInterfaceCfgXml::CInterfaceCfgXml( CString & strLang, BOOL bDECODE/*=FALSE*/ )
	: m_strLang(strLang)
	, m_bDecode(bDECODE)
{
	m_XmlDoc.SetXMLCrypt( bDECODE? true:false );
}
		...
		while(ControlElem)
		{
			...
			{ // Note : 텍스쳐를 얻는다.
				...
				if(TextureElem)
				{
					...
					MultiByteToWideChar(CP_UTF8,0,szValue,(int)TextureElem->FirstChild()->ValueTStr().size()+1,szTemp,sizeof(szTemp)/sizeof(szTemp[0]));			
