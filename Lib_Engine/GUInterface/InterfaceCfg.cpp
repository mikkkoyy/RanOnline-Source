...
BOOL CInterfaceCfg::LoadText( const TCHAR * filename, BOOL bDECODE/*=FALSE*/ )
{
	CString strFileName;
	strFileName = GetPath();
	...
	//{
		CInterfaceCfgXml interfaceCfgXml( RANPARAM::strCountry, bDECODE );
		return interfaceCfgXml.Parse( m_mapUICfg, strFileName.GetString() );
	/*}
	else
	{
		CStringFile StrFile;
		if ( !StrFile.Open ( strFileName.GetString() ), bDECODE )	return FALSE;
