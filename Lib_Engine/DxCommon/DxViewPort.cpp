...
HRESULT DxViewPort::SetDefaultCamera ()
{
	m_bHandling = true;
	m_dwFlag = (DXVP_USER|DXVP_LANDSCAPEMOVE);
	m_fZOOMIN = 30.0f;
...
void DxViewPort::CameraZoom ( float fZoomOut )
{
	...
	m_fLength += ( fZoomOut * 0.6f );
	if ( (m_fLength/0.6f) < m_fZOOMIN )		m_fLength = ( m_fZOOMIN * 0.6f );
	else if ( m_bGame )
	{
		const float fZoomOut = 200.0f;
		if( (m_fLength/0.6f) > fZoomOut )	m_fLength = (fZoomOut * 0.6f);
	}
	else
	{
		const float fZoomOut = 1800.0f;
		if( (m_fLength/0.6f) > fZoomOut )	m_fLength = (fZoomOut * 0.6f);
	}
	...
	//	Note : 카메라 줌.
	m_vFromPt = m_vLookatPt - vCameraDir * ( m_fLengthModify / 0.6f ) ;
