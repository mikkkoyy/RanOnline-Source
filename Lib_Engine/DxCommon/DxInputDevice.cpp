...
void DxInputDevice::UpdateKeyState  ()
{
	...
	for ( DWORD i = 0; i < m_dwKeyElements; i++ )
	{
		...
		{
			/*quick slot delay, Juver, 2017/10/29 */
			if ( m_dwLastKeyboardTime[nScanCode] != 0 &&
				m_KeyDidodata[i].dwTimeStamp - m_dwLastKeyboardTime[nScanCode] < 100 )
			{
				if ( nScanCode == RANPARAM::QuickSlot[0] || 
					nScanCode == RANPARAM::QuickSlot[1] || 
					nScanCode == RANPARAM::QuickSlot[2] ||
					nScanCode == RANPARAM::QuickSlot[3] || 
					nScanCode == RANPARAM::QuickSlot[4] || 
					nScanCode == RANPARAM::QuickSlot[5] )
				{
					RANPARAM::dwPotsCount++;
					continue;
				}
			}
