...
HRESULT DxLandGate::Load ( basestream	&SFile )
{
	...
	if( dwVer == 0x0103 )
	{
		...
		{
			SFile.ReadBuffer ( &szName, sizeof(char)*MAX_SZNAME );
		}
		...
		SNATIVEID		sidToMapID[wNUM];
		for( int i=0; i<wNUM; ++i )
		{
			SFile >> sidToMapID[i].dwID;
		}

		Prop.m_ToMapID.dwID = sidToMapID[0].dwID;
	...
	else if ( dwVer == 0x0101 )
	{
		...
		SFile.ReadBuffer ( &m_Prop.m_szName, sizeof(char) * 68 );
		SFile >> m_Prop.m_GateID;
		SFile >> m_Prop.m_dwGeteFlags;

		SFile >> m_Prop.m_dwStartPNum;

		//SFile >> m_Prop.m_ToMapID;
		SFile.ReadBuffer ( &m_Prop.m_ToMapID, sizeof(SNATIVEID) );
		SFile >> m_Prop.m_ToGateID;
		//SFile >> m_Prop.m_vDiv;
		SFile.ReadBuffer ( &m_Prop.m_vDiv, sizeof(D3DXVECTOR2) );
		SFile >> m_Prop.m_vMin;
		SFile >> m_Prop.m_vMax;
	...
	// | 15-02-2013 | add new wld version
