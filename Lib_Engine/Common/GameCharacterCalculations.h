...
namespace GameCharacterCalculations
{
    ...
    // ---------------------------------------------------------------------------
    // ItemAttackVelocityRatio — item attack-speed ratio.
    //
    ...
    //
    // The caller is responsible for:
    //   - obtaining itemAttackSpeed (m_sSUMITEM.fInc_AtkSpeed)
    //   - all surrounding attack-velocity pipeline (GETATTVELO, etc.)
    //
    // The portable function does NOT generate randomness. It only performs
    ...
    // ---------------------------------------------------------------------------
    // TaxiCharge — taxi charge arithmetic.
    //
    ...
    //
    // The caller is responsible for:
    //   - obtaining baseCharge (sTaxiStation.GetBasicCharge())
    //   - obtaining mapCharge (pSTATION->dwMapCharge)
    //   - resolving differentMap (pSTATION->dwMAPID != dwCurMapID)
    //   - obtaining shopBuyRate (GetBuyRate())
    //   - resolving applyShopRate (an active land exists)
    //   - all surrounding taxi/station/map resolution
    //
    // The portable function does NOT generate randomness. It only performs
    ...
    // ---------------------------------------------------------------------------
    // MoveVelocityModifier — movement velocity modifier.
    //
    ...
    //
    // The caller is responsible for:
    //   - obtaining stateMove (m_fSTATE_MOVE)
    //   - obtaining skillMove (m_fSKILL_MOVE)
    //   - obtaining optionMove (m_fOPTION_MOVE; 0.0f for NPC)
    //   - obtaining itemMoveRate (m_sSUMITEM.fIncR_MoveSpeed; 0.0f for NPC)
    //   - all surrounding movement pipeline
    //
    // The portable function does NOT generate randomness. It only performs
    ...
    // ---------------------------------------------------------------------------
    // PkLevel — PK level threshold calculation.
    //
    ...
    //
    // The caller is responsible for:
    //   - obtaining brightness (m_nBright)
    //   - obtaining pkPoints (GLCONST_CHAR::sPK_STATE[].nPKPOINT)
    //   - obtaining pkStateCount (GLCONST_CHAR::EMPK_STATE_LEVEL)
    //   - all surrounding PK state table ownership
    //
    // The portable function does NOT generate randomness. It only performs
    ...
    // ---------------------------------------------------------------------------
    // IsOffender — PK offender predicate.
    //
    ...
    // The caller is responsible for:
    //   - obtaining brightness (m_nBright)
    //   - obtaining offenderThreshold (GLCONST_CHAR::sPK_STATE[0].nPKPOINT)
    //   - all surrounding PK gameplay behavior
    //
    // The portable function does NOT generate randomness. It only performs
