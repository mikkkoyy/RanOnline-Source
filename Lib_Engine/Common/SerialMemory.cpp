...
CSerialMemory::CSerialMemory ( const char *_szZipFileName, const char *_szFileName, EMBYTECRYPT emCrypt /*= EMBYTECRYPT_NONE*/ ) 
	: m_pBuffer(NULL)
	, m_nBufSize(0)
	, m_dwOffSet(0)
{
	...
	OpenFile ( _szZipFileName, _szFileName, emCrypt );
}
...
BOOL CSerialMemory::OpenFile (	const char *_szZipFileName, const char *_szFileName, EMBYTECRYPT emCrypt /*= EMBYTECRYPT_NONE*/, BOOL bPack /*= FALSE*/ )
{
	//GASSERT(_szFileName);
	//GASSERT(_szZipFileName);
