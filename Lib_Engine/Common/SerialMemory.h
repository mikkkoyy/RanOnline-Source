...
public:
	...
	//wld crypt
	virtual void SetEncode ( EMBYTECRYPT emCrypt, EMENCODE_TYPE emType = EMENCODE_NONE )		{ m_emCrypt = emCrypt; m_emEncodeType = emType; }
	virtual EMENCODE_TYPE	GetEncodeType()		{ return m_emEncodeType; }

	virtual void SetEncodeType ( EMBYTECRYPT emCrypt )		{ m_emCrypt = emCrypt; }

	BOOL OpenFile ( const char *_szZipFileName, const char *_szFileName, EMBYTECRYPT emCrypt = EMBYTECRYPT_NONE, BOOL bPack = FALSE );
...
public:
	CSerialMemory ();
	CSerialMemory ( const char *_szZipFileName, const char *_szFileName, EMBYTECRYPT emCrypt = EMBYTECRYPT_NONE );
