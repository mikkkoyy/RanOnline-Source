...
const DWORD DxPieceEdit::VERSION_PIE	= 0x0103;
...
BOOL DxPieceEdit::LOADPIE_0101 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_vMin;
	SFile >> m_vMax;
	SFile.ReadBuffer( &m_matLocal, sizeof(D3DXMATRIX) );
...
BOOL DxPieceEdit::LOADPIE_0105_6 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_vStaticMin;
	SFile >> m_vStaticMax;
	SFile >> m_vMin;
	...
	if ( bExist )
	{
		...
		DWORD dwCur = SFile.GetfTell();
		SFile.SetOffSet( dwCur+_dwBufferSize );
...
BOOL DxPieceEdit::LoadPIE_0107( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const TSTRING& strPieceEditPath )
{
	DWORD dwData(0);
...
BOOL DxPieceEdit::LoadPIE_0108( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const TSTRING& strPieceEditPath )
{
	DWORD dwData(0);
	std::string strFrameName;
	float fData(0.0f);
...
BOOL DxPieceEdit::LoadPIE_0109( CSerialFile& SFile )
{
	DWORD dwData(0);
	std::string strFrameName;
	float fData(0.0f);
...
HRESULT DxPieceEdit::LoadPIE( LPDIRECT3DDEVICEQ pd3dDevice, const char* szName )
{
	...
	{
		LoadPIE_0108( pd3dDevice, SFile, "" );
	}
	else if( dwVer==0x0107 )
	{
		LoadPIE_0107( pd3dDevice, SFile, "" );
	}
	else if( dwVer == 0x0104 || dwVer == 0x0105 || dwVer == 0x0106 )
	{
		LOADPIE_0105_6( SFile, pd3dDevice );
	}
	else if( dwVer==VERSION_PIE )
	{
