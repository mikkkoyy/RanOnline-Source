...
public:
	...
	void Load_File( LPDIRECT3DDEVICEQ pd3dDevice, const char* szName, EMENCODE_TYPE emType = EMENCODE_NONE );	//wld crypt
	void Load_File_119( LPDIRECT3DDEVICEQ pd3dDevice, const char* szName );

	// Dynamic Loading	|| General <SFile==NULL> || Piece File			Loading 을 의미함.
	void Save( CSerialFile& SFile, BOOL bPiece, BOOL bFromFile = FALSE );
	void Load( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const BOOL bDynamicLoad, const BOOL bPiece, BOOL bFromFile = FALSE );
	void Load_119( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const BOOL bDynamicLoad, const BOOL bPiece, BOOL bFromFile = FALSE );
...
protected:
	...
	void SingleTexMesh_Load_Insert_119( LPDIRECT3DDEVICEQ pd3dDevice, MAPSINGLETEXMESH& mapMesh, CSerialFile& SFile, BOOL bDynamicLoad, BOOL bPiece );
