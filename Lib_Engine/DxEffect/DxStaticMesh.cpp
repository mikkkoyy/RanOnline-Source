...
void DxStaticMesh::Save_File ( const char* szName )
{
	...
	if ( m_SFile.OpenFile ( FOT_WRITE, strName.c_str() ) )	
		Save( m_SFile, FALSE, TRUE );

	m_SFile.CloseFile();
}

void DxStaticMesh::Load_File ( LPDIRECT3DDEVICEQ pd3dDevice, const char* szName, EMENCODE_TYPE emType )
{
	std::string strName = szName;
	...
	{
		//wld crypt
		if( emType != EMENCODE_NONE )	m_SFile.SetEncode( EMBYTECRYPT_OLD, EMENCODE_WLD0 );
		Load( pd3dDevice, m_SFile, TRUE, FALSE, TRUE );
	}
}

void DxStaticMesh::Load_File_119 ( LPDIRECT3DDEVICEQ pd3dDevice, const char* szName )
{
	std::string strName = szName;
	std::string strName1 = szName;
	...
	{
		std::string::size_type idx;
		idx = strName1.find( ".wld" );
		if( idx!=std::string::npos )	strName1 += '1';		// wld1 로 만듬.

		CSerialFile SFile;	
		if ( SFile.OpenFile ( FOT_READ, strName1.c_str() ) )	
			Load_119( pd3dDevice, SFile, TRUE, FALSE, TRUE );
...
void DxStaticMesh::Save ( CSerialFile& SFile, BOOL bPiece, BOOL bFromFile /*= FALSE*/ )
{
	DWORD	dwSize = 0;

	SFile << (DWORD)VERSION_WLD0;
...
void DxStaticMesh::Load( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const BOOL bDynamicLoad, const BOOL bPiece, BOOL bFromFile /*= FALSE*/ )
{
	CleanUp();	// 모두 초기화
	...
	if( dwVer==VERSION_WLD0 )
	{
		SFile >> m_vMax;
		SFile >> m_vMin;

		//if ( bFromFile && dwVer >= VERSION_ENCODE )
		//	SFile.SetEncodeType( EMBYTECRYPT_WLD );
...
void DxStaticMesh::Load_119( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const BOOL bDynamicLoad, const BOOL bPiece, BOOL bFromFile /*= FALSE*/ )
{
	DWORD dwVer, dwBufferSize;
	SFile >> dwVer;
	SFile >> dwBufferSize;

	if( dwVer==VERSION_WLD1 )
	{
		SingleTexMesh_Load_Insert_119( pd3dDevice, m_mapMesh, SFile, bDynamicLoad, bPiece );
	}
	else
	{
		CDebugSet::ErrorVersion( "DxStaticMesh::Load_119", SFile.GetFileName(), dwVer );
		DWORD dwCur = SFile.GetfTell();
		SFile.SetOffSet( dwCur+dwBufferSize );
		return;
	}
...
void DxStaticMesh::SingleTexMesh_Load_Insert_104( LPDIRECT3DDEVICEQ pd3dDevice, MAPSINGLETEXMESH& mapMesh, CSerialFile& SFile, BOOL bDynamicLoad, BOOL bPiece )
{
	...
	for ( DWORD i=0; i<dwSize; ++i )
	{
		...
		DWORD dw(0); //flag for name formating by official
		SFile >> dw;
...
void DxStaticMesh::SingleTexMesh_Load_Insert_119( LPDIRECT3DDEVICEQ pd3dDevice, MAPSINGLETEXMESH& mapMesh, CSerialFile& SFile, BOOL bDynamicLoad, BOOL bPiece )
{
	if( bPiece )	m_emRenderType = EMRT_PIECE;
	else			m_emRenderType = EMRT_OCTREE;
	...
	for ( DWORD i=0; i<dwSize; ++i )
	{
		...
		DxSingleTexMesh* pOrigMesh = FindMesh( szName.c_str() );
		if ( pOrigMesh )
		{
			pOrigMesh->Load_EP9( pd3dDevice, SFile, bDynamicLoad, bPiece );
		}
		else
		{
			DxSingleTexMesh* pNew = new DxSingleTexMesh;
			pNew->Load_EP9( pd3dDevice, SFile, bDynamicLoad, bPiece );
